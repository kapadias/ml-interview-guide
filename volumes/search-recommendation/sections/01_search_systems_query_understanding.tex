...
\begin{tikzpicture}[
    ...
    \node[stage, fill=lightyellow, minimum width=10.4cm, text width=10.0cm, below=0.5cm of query] (qu)
    ...
    % Feedback loop (right side; lane sits clear of the widest stage)
    \path (qu.east) ++(0.65,0) coordinate (fbLane);
    \coordinate (fbA) at (fbLane |- serp.east);
    \coordinate (fbB) at (fbLane |- query.east);
    \draw[->, dashed, thick, deepred] (serp.east) -- (fbA) -- (fbB) -- (query.east);
    \node[fb, rotate=90, anchor=center] at ($(fbA)!0.5!(fbB)+(0.35,0)$)
