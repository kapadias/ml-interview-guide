...
\subsection{Next Generation: Does the Taxonomy Survive Generative Ranking?}
\label{sr:subsec:ltr_generative}

This section opened by insisting the taxonomy is a statement about \textit{loss structure}, not model architecture. Generative ranking is the case that tests the claim, because it changes what a loss can be structured over. In pointwise autoregressive ranking (Section~\ref{sr:subsec:autoregressive_ranking}) the model emits a document identifier token by token and ranking falls out of beam search, so the loss unit is a \textit{token}, not a (query, document) pair, and there is no candidate set to be listwise over in the first place.

Run the standard recipe through the taxonomy and it lands badly. Next-token prediction over docIDs is \textbf{rank-agnostic}: it supervises the single correct continuation and treats every other identifier as an undifferentiated negative. That is structurally \textit{weaker} than pointwise regression, which at least fits a graded label per document---a top-1 objective in a chapter's worth of evidence that top-1 is not the product. Rozonoyer et al.'s \textbf{SToICaL} (arXiv:2601.05588) repairs it with item-level reweighting plus prefix-tree marginalization, distributing probability mass across valid docID tokens in proportion to ground-truth relevance. The result is pointwise by architecture and listwise by supervision, which is not a cell the table above has.

\begin{keyinsight}[The Axis Survives; the Three Families Do Not]
What the taxonomy is really measuring is \textit{how much of the rest of the list the loss can see}---and that axis transfers intact to objectives with no candidate set. The family names do not: ``pointwise'' describes SToICaL's architecture and ``listwise'' describes its supervision, and both are true at once. Carrying the axis rather than the labels is what lets you reason about an objective the taxonomy was not built for, which is the actual L6+ skill the pointwise/pairwise/listwise question probes.
\end{keyinsight}

\textbf{The LambdaRank parallel, and where it breaks.} Item-level reweighting is recognizably the lambda move of Section~\ref{sr:sec:lambda_family}---do not treat every update equally, scale it by how much the item deserves to move---pushed down to token level. But the analogy stops at an instructive place. LambdaRank's weights are $|\Delta\text{NDCG}|$ computed from the documents' \textit{current predicted positions}, so the gradient is metric-aware and position-dependent, and it re-weights itself as the model improves. SToICaL's weights come from ground-truth grades alone. It is graded-relevance-aware supervision, not metric-aware supervision, and it therefore inherits the same gap the permutation-likelihood losses have: nothing in the objective knows that an inversion at ranks 1--2 costs more than one at 49--50. A lambda-style, position-discounted reweighting for generative rankers is an open problem, and saying so is a cleaner demonstration of understanding the lambda trick than restating it.

Two further asymmetries are worth knowing. \textbf{Prefix-tree marginalization has no LTR analogue}: it exists because identifiers are token sequences and probability mass leaks onto prefixes no valid document completes. That machinery is doing validity and calibration at once (Section~\ref{sr:subsec:rank_aware_generative}). And the calibration warning above transfers with a twist---a generative ranker emits a proper probability distribution, but over an \textit{identifier vocabulary}, not over relevance, so its softmax is no more a calibrated $P(\text{relevant})$ than a listwise reranker's logits are. Anywhere money or a threshold touches the score, the pattern of the previous key insight is unchanged: a calibrated pointwise head where the number must mean something, the ranking objective on top.
...
\badgeLfive{L5} \quad \badgetype{System Design} \quad \badgefreq{\freqmed}
