...
\begin{tikzpicture}[scale=1.0, every node/.style={font=\small}]
    % Triangle (widened so the placements sit clear of the edges and of each other)
    \coordinate (R) at (0, 4.6);
    \coordinate (L) at (-4.9, -0.5);
    \coordinate (M) at (4.9, -0.5);
    ...
    % placements
    \node[deepgreen] at (0, 3.1) {Flat (exact)};
    \node[font=\footnotesize, deepgreen, align=center] at (0, 2.5)
        {recall $= 1$,\\ pay latency or memory};
    \node[deepred] at (-2.2, 1.2) {HNSW};
    \node[font=\footnotesize, deepred, align=center] at (-2.2, 0.6)
        {recall $+$ speed,\\ pay RAM};
    \node[orange!80!black] at (2.2, 1.2) {IVF-PQ};
    \node[font=\footnotesize, orange!80!black, align=center] at (2.2, 0.6)
        {memory $+$ speed,\\ pay recall};
    \node[deepblue] at (0, -1.5) {DiskANN: buy back memory with NVMe latency \quad $\cdot$ \quad rerank: buy back recall with oversampling};
\end{tikzpicture}
