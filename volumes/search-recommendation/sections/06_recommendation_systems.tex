...
\begin{tikzpicture}[
    node distance=0.5cm,
    box/.style={rectangle, draw, minimum width=1.4cm, minimum height=0.6cm, rounded corners=2pt, font=\small},
    ...
    \node[tower] (sb_t1) at (-0.85, 2.0) {Task 1};
    \node[tower] (sb_t2) at (0.85, 2.0) {Task 2};
    \draw[->] (sb_shared) -- (sb_t1);
    \draw[->] (sb_shared) -- (sb_t2);

    % ---- MMoE ----
    \node[label] at (5, 4.5) {MMoE};
    \node[expert] (m_e1) at (3.3, 3.3) {Exp 1};
    \node[expert] (m_e2) at (5, 3.3) {Exp 2};
    \node[expert] (m_e3) at (6.7, 3.3) {Exp 3};
    \node[gate] (m_g1) at (4.15, 2.3) {Gate 1};
    \node[gate] (m_g2) at (5.85, 2.3) {Gate 2};
    \node[tower] (m_t1) at (4.15, 1.3) {Task 1};
    \node[tower] (m_t2) at (5.85, 1.3) {Task 2};
    \draw[->, gray] (m_e1) -- (m_g1);
    \draw[->, gray] (m_e2) -- (m_g1);
    \draw[->, gray] (m_e3) -- (m_g1);
    \draw[->, gray] (m_e1) -- (m_g2);
    ...
    % ---- PLE ----
    \node[label] at (10.9, 4.5) {PLE};
    \node[expert, fill=blue!20] (p_s1) at (10.9, 3.3) {Shared};
    \node[expert, fill=red!15] (p_e1) at (9.2, 3.3) {Task-1};
    \node[expert, fill=green!15] (p_e2) at (12.6, 3.3) {Task-2};
    \node[gate] (p_g1) at (10.05, 2.3) {Gate 1};
    \node[gate] (p_g2) at (11.75, 2.3) {Gate 2};
    \node[tower] (p_t1) at (10.05, 1.3) {Task 1};
    \node[tower] (p_t2) at (11.75, 1.3) {Task 2};
    \draw[->, gray] (p_s1) -- (p_g1);
    \draw[->, gray] (p_s1) -- (p_g2);
    \draw[->, gray] (p_e1) -- (p_g1);
    \draw[->, gray] (p_e2) -- (p_g2);
...
\subsection{Next Generation: Rank-Aware Training Beyond Top-1}
\label{sr:subsec:rank_aware_generative}

The table above is the 2026 state of practice. The open research problem it hides is that generative retrieval is trained with the wrong objective for ranking. TIGER-style models are fine-tuned with next-token prediction over semantic-ID tokens, which is a \textbf{top-1 objective}: it optimizes for emitting the single best item's code sequence and supervises nothing about the order of the rest of the list. That is tolerable when the deliverable is one next-item prediction and a real problem when the deliverable is a slate.

Rozonoyer et al.\ (arXiv:2601.05588; Google DeepMind, UMass Amherst, UT Austin) attack exactly this. Their \textbf{SToICaL} loss (Simple Token-Item Calibrated Loss) adds item-level reweighting and prefix-tree marginalization, distributing probability mass across valid docID tokens in proportion to graded relevance instead of concentrating it on one. On WordNet and ESCI (Amazon Shopping Queries) it suppresses invalid-ID generation and improves ranking metrics beyond top-1---with the honest caveat that one variant degraded top-1 in the process. The same paper proves the expressivity case for generation over dual encoders (a DE needs embedding dimension linear in corpus size for arbitrary rankings, a generative ranker needs constant hidden dimension); that argument and its serving-side limits are treated in Section~\ref{sr:subsec:autoregressive_ranking}.

\begin{keyinsight}[Constrained Decoding Is Doing Two Jobs]
Section~\ref{sr:sec:llm_recsys} introduces prefix-constrained decoding as the fix for \textit{hallucination}---keeping the model from emitting item IDs that do not exist. The rank-aware losses use the same prefix tree as a \textit{ranking} mechanism: marginalizing over the valid-ID tree is how graded relevance gets pushed down to token-level supervision. Recognizing that the same machinery serves validity and calibration is the distinction between having read about generative retrieval and having reasoned about it.
\end{keyinsight}

For interviews, the useful synthesis is a sequencing one. Semantic IDs inside conventional rankers (the YouTube path) are deployable now and pay off on cold-start and tail. End-to-end generative retrieval is still gated on serving cost, catalog churn, and---per this line of work---an objective that ranks rather than merely predicts. A candidate who can name which of those three is closest to being solved is reading the field rather than the headlines.
