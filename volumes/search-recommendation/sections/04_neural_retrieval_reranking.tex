...
\subsection{On the Horizon: Autoregressive Ranking (2026)}
\label{sr:subsec:autoregressive_ranking}

Everything so far in this chapter sits on one axis: how much query--document interaction you can afford, from the bi-encoder's fully decomposed dot product, through late interaction, to the cross-encoder's full attention. \textbf{Autoregressive ranking (ARR)} proposes a different axis entirely---an LLM generates document identifiers token by token, and ranking falls out of beam search over docIDs. There is no embedding table and no ANN index; the decoder is the index. This is not production practice in 2026 and should not be proposed as a default, but the 2026 result below is the cleanest statement yet of \textit{why} the funnel is shaped the way it is, which makes it worth knowing at L6+.

\textbf{The expressivity result.} Rozonoyer et al.\ (\textit{Autoregressive Ranking: Bridging the Gap Between Dual and Cross Encoders}, arXiv:2601.05588; Google DeepMind, UMass Amherst, UT Austin) prove that ARR's expressive capacity is strictly greater than a dual encoder's: realizing \textit{arbitrary} rankings over a corpus forces a DE's embedding dimension to grow \textbf{linearly with corpus size}, while ARR achieves it at \textbf{constant} hidden dimension.

\begin{keyinsight}[The Decomposability Contract Has a Price Tag]
Section~\ref{sr:subsec:bi_encoder_contract} states the bi-encoder's contract qualitatively: score decomposition buys sublinear retrieval and costs expressiveness. The ARR result prices that cost. The bi-encoder's ceiling is not an optimization failure and not a data shortage---it is the rank of a bilinear form, and the dimension required scales with how much ranking structure one dot product is asked to carry. This is the formal reason a multi-stage funnel exists at all: you cannot buy your way out of it with a bigger encoder, so you buy interaction back at the top of the funnel where the candidate count makes it affordable.
\end{keyinsight}

\textbf{SToICaL: the training half.} The paper's second contribution is the more portable one. Fine-tuning an LLM to rank with standard next-token prediction uses a \textbf{rank-agnostic} objective---a top-1 loss that says nothing about the order of everything below the first docID. SToICaL (Simple Token-Item Calibrated Loss) adds item-level reweighting and prefix-tree marginalization so probability mass is distributed across valid docID tokens in proportion to graded relevance. On WordNet and ESCI (Amazon Shopping Queries), it suppresses invalid docID generation and lifts ranking metrics \textit{beyond} top-1.

Note the continuity with the rest of this chapter: the distillation argument in Section~\ref{sr:subsec:ce_distillation} is that a binary relevance label carries one bit while a teacher's score distribution carries the whole ordering. SToICaL is that same substitution---rank-agnostic objective for rank-calibrated one---applied to a generative model. The lesson generalizes past the architecture, and it is the part to reach for when an interviewer asks what LLM-era ranking actually changed.

\begin{warningbox}
\textbf{Do not overstate this in an interview.} It is a theory paper with two academic benchmarks. It reports \textit{no} latency or serving-cost comparison against DE+ANN or cross-encoders, and on shopping data one variant degraded top-1 while improving the rest of the list. Beam search over a large docID space on a consumer-latency hot path has no published recipe, corpus churn is unsolved (a new document is a docID the decoder was never trained to emit), and hard filters---in-stock, permissions, locale---have no ANN-equivalent story. The defensible L6 answer cites the expressivity result to explain the funnel, then declines ARR for a serving path on exactly these grounds.
\end{warningbox}
