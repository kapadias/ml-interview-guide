...
\begin{enumerate}[label=\textbf{\arabic*.}]
    ...
    and word frequencies are wildly skewed. \textbf{Zipf's law} makes this quantitative:
    the frequency of the $r$-th most frequent word follows a power law,
    $f(r) \propto r^{-s}$ with $s \approx 1$, so the second-ranked word appears roughly
    half as often as the first, the tenth roughly a tenth as often, and the tail of rare
    words stretches on indefinitely (in the Brown corpus, ``the'' alone accounts for about
    7\% of all tokens, and the ten most frequent word types cover roughly a quarter of the
    text). The companion result, \textbf{Heaps' law}, says vocabulary growth never
    saturates: the number of distinct word types in a corpus of $N$ tokens grows as
    $V(N) \approx k N^{\beta}$ with $\beta \approx 0.4$--$0.6$, so doubling the corpus
    always surfaces new words. Together they imply that most of the events a count-based
    model must estimate are rare or entirely unseen---the root cause of the smoothing
    machinery of Chapter~\ref{chap:statistical_nlp}, and the reason out-of-vocabulary
    handling is a permanent design concern rather than a corner case.
...
\subsection{Classical NLP Inside the 2026 LLM Stack}
\label{sec:classical_llm_stack}

The warning above has a sharper, more current form. The strongest 2026 evidence that
classical NLP still matters is not that BM25 powers Elasticsearch---it is that classical
methods are load-bearing \emph{inside the data pipelines that build frontier LLMs}. Every
serious pretraining effort filters, deduplicates, and decontaminates a raw web corpus
before spending a single GPU-hour on training, and that stage is built almost entirely
from this chapter's machinery:

\begin{itemize}
    \item \textbf{Linear quality classifiers at trillion-token scale.} The
    best-performing quality filter in DataComp-LM (DCLM, 2024) is a fastText
    classifier---a linear model over averaged word and bigram embeddings---trained to
    separate known-high-quality text (instruction data, curated answers) from random web
    pages; it beat far more elaborate filtering schemes. FineWeb-Edu follows the same
    pattern with a small regression head on frozen sentence embeddings, trained on
    LLM-graded ``educational value'' labels. In both cases a simple linear model scores
    billions of documents on CPUs; the LLM is used only to label a few hundred thousand
    training examples, never to score the corpus.

    \item \textbf{KenLM perplexity filtering.} The CCNet pipeline (and the
    LLaMA-lineage corpora built on it) scores every web document with a Kneser-Ney
    5-gram model (KenLM) trained on Wikipedia, then buckets documents into
    head/middle/tail by perplexity: text that a clean-corpus n-gram model finds
    surprising is disproportionately boilerplate, gibberish, or spam. The n-gram
    language model of Chapter~\ref{chap:statistical_nlp}, retired as a generator,
    thrives as a data-quality instrument.

    \item \textbf{MinHash/Jaccard deduplication over n-gram shingles.} Web crawls are
    massively duplicated, and duplicated training text drives memorization, wastes
    compute, and can leak evaluation data many times over. Near-duplicate detection
    represents each document as its set of $n$-gram shingles (\S\ref{sec:ngrams}),
    estimates Jaccard similarity between documents with MinHash signatures, and groups
    candidates with locality-sensitive hashing---classical IR machinery, run at
    trillion-token scale in essentially every frontier data pipeline.

    \item \textbf{N-gram overlap decontamination.} Benchmark integrity is enforced by
    scanning the training corpus for long n-gram collisions with evaluation sets (the
    GPT-3 report used 13-gram overlap; variants of the same test remain standard).
    Without it, reported benchmark gains can be memorization of leaked test data.
\end{itemize}

\begin{keyinsight}[The 10T-Token Corpus Question]
When a frontier-lab interviewer asks ``how would you filter and deduplicate a
10-trillion-token pretraining corpus?'', they are asking a classical NLP question. The
production answer---fastText quality classifiers, KenLM perplexity filters, MinHash over
n-gram shingles, n-gram overlap decontamination---is assembled from the representations
and models in this chapter and the next, chosen precisely for the classical virtues this
chapter emphasizes: CPU-speed throughput, interpretable scores, and no requirement for
labeled data beyond corpus statistics. A worked design for this question closes this
chapter (Q10); the end-to-end pipeline treatment (data mixtures, curricula, scaling-law
practice, infrastructure) lives in the Deep Learning volume's \emph{Pretraining at Scale}
chapter [DL].
\end{keyinsight}
...
\begin{tikzpicture}[
    ...
    cat/.style={draw=deepblue!70, fill=lightblue, rounded corners=3pt,
                font=\small\sffamily, minimum width=3.1cm, minimum height=0.85cm, align=center},
    task/.style={draw=gray!50, fill=white, rounded corners=2pt,
                 font=\scriptsize\sffamily, minimum width=2.9cm, minimum height=0.5cm, align=center},
    link/.style={draw=deepblue!60, thick, -{Stealth[length=2mm]}},
    spine/.style={draw=deepblue!45, thick}
]
% Root
\node[root] (root) at (0,1.1) {NLP Tasks};

% Four categories, one column each
\node[cat] (c1) at (-5.1,-0.5) {Text\\Classification};
\node[cat] (c2) at (-1.7,-0.5) {Sequence\\Labeling};
\node[cat] (c3) at ( 1.7,-0.5) {Seq-to-Seq};
\node[cat] (c4) at ( 5.1,-0.5) {Structured\\Prediction};
\foreach \c in {c1,c2,c3,c4}{ \draw[link] (root) -- (\c); }

% Example tasks, stacked below their category
\node[task] at (-5.1,-1.75) {Sentiment};
\node[task] at (-5.1,-2.40) {Topic};
\node[task] at (-5.1,-3.05) {Intent};
\node[task] at (-1.7,-1.75) {NER};
\node[task] at (-1.7,-2.40) {POS};
\node[task] at (-1.7,-3.05) {Chunking};
\node[task] at ( 1.7,-1.75) {Translation};
\node[task] at ( 1.7,-2.40) {Summarization};
\node[task] at ( 1.7,-3.05) {QA};
\node[task] at ( 5.1,-1.75) {Parsing};
\node[task] at ( 5.1,-2.40) {Coreference};
\node[task] at ( 5.1,-3.05) {Relation Ext.};

% Bracket from each category down its own column of tasks
\foreach \x in {-5.1,-1.7,1.7,5.1}{
    \draw[spine] (\x,-0.925) -- (\x,-1.13) -- (\x-1.73,-1.13) -- (\x-1.73,-3.05);
    \foreach \y in {-1.75,-2.40,-3.05}{
        \draw[spine] (\x-1.73,\y) -- (\x-1.45,\y);
    }
}
\end{tikzpicture}
...
\centering
\begin{tikzpicture}[
    pipestage/.style={draw=deepblue, fill=lightblue, rounded corners=4pt,
    ...
    \node[pipestage] (raw) at (0,0) {Raw Text};
    \node[pipestage] (enc) at (3.2,0) {Encoding\\Normalization};
    \node[pipestage] (uni) at (6.4,0) {Unicode\\Normalization};
    \node[pipestage] (tok) at (9.6,0) {Tokenization};
    \node[pipestage] (case) at (0,-2.2) {Case\\Normalization};
    \node[pipestage] (stop) at (3.2,-2.2) {Stopword\\Removal};
    \node[pipestage] (stem) at (6.4,-2.2) {Stemming /\\Lemmatization};
    \node[pipestage] (feat) at (9.6,-2.2) {Feature\\Extraction};
...
represents it as the corresponding standard basis vector: for $|\mathcal{V}| = V$, word
$w_i$ becomes $\mathbf{e}_i \in \{0, 1\}^V$ with a single 1 in position $i$. That is the
substrate the rest of this section builds on---bag of words sums one-hot vectors over a
document, TF-IDF reweights that sum---while in modern systems it survives only as the
lookup index into an embedding matrix. Its fatal flaws (mutual orthogonality, so ``cat''
is exactly as far from ``dog'' as from ``quantum''; $V$-dimensional sparsity; no
generalization from one word to a related one) are precisely what motivate distributional
representations, and they are argued once, where they do their work, in
Chapter~\ref{chap:word_representations}.
...
Here $n$-grams are \emph{features}: counts of contiguous token sequences handed to a
classifier or an index. The other use of the same statistics---as a probabilistic language
model estimating $P(w_i \given w_{i-n+1}, \ldots, w_{i-1})$, with the smoothing and
back-off machinery that requires, scored by perplexity---belongs to
Chapter~\ref{chap:statistical_nlp} (Sections~\ref{sec:ngram_lm} and~\ref{sec:perplexity}).
...
\begin{warningbox}
A known gotcha: the classic probabilistic idf in Eq.~\eqref{eq:bm25_idf} becomes
\emph{negative} whenever $\text{df}(q_i) > N/2$---a term appearing in more than half the
documents---so matching such a term would \emph{lower} a document's score. Production
implementations guard against this: Lucene (and therefore Elasticsearch) computes
$\text{idf}(q_i) = \log\bigl(1 + \frac{N - \text{df}(q_i) + 0.5}{\text{df}(q_i) + 0.5}\bigr)$,
adding 1 inside the logarithm precisely so the idf stays positive.
\end{warningbox}
...
\subsubsection{Worked Example: Saturation and Length Normalization in Action}

Consider a corpus of $N = 1000$ documents with average length $d_{\text{avg}} = 100$
tokens, the query ``transformer models,'' and the defaults $k_1 = 1.2$, $b = 0.75$
(natural logarithms throughout; the base only rescales all scores):

\begin{itemize}
    \item ``transformer'': $\text{df} = 50$, so
          $\text{idf} = \ln\frac{1000 - 50 + 0.5}{50 + 0.5} = \ln(18.8) \approx 2.94$.
    \item ``models'': $\text{df} = 400$, so
          $\text{idf} = \ln\frac{600.5}{400.5} \approx 0.41$---the common term
          contributes little (and past $\text{df} > 500$, Eq.~\eqref{eq:bm25_idf}
          would go \emph{negative}).
\end{itemize}

\textbf{Document A} (short and focused): $|d| = 50$, $f_{\text{transformer}} = 3$,
$f_{\text{models}} = 2$. The length factor is
$1 - b + b\,\frac{|d|}{d_{\text{avg}}} = 0.25 + 0.75 \times 0.5 = 0.625$, so the TF
denominator adds $k_1 \times 0.625 = 0.75$:
\[
\text{tf}_{\text{transformer}} = \frac{3 \times 2.2}{3 + 0.75} = 1.76, \qquad
\text{tf}_{\text{models}} = \frac{2 \times 2.2}{2 + 0.75} = 1.60,
\]
\[
\text{BM25}(Q, A) = 2.94 \times 1.76 + 0.41 \times 1.60 \approx 5.8 .
\]

\textbf{Document B} (four times the raw counts, but long): $|d| = 200$,
$f_{\text{transformer}} = 12$, $f_{\text{models}} = 4$. The length factor is
$0.25 + 0.75 \times 2 = 1.75$, so the denominator adds $k_1 \times 1.75 = 2.1$:
\[
\text{tf}_{\text{transformer}} = \frac{12 \times 2.2}{12 + 2.1} = 1.87, \qquad
\text{tf}_{\text{models}} = \frac{4 \times 2.2}{4 + 2.1} = 1.44,
\]
\[
\text{BM25}(Q, B) = 2.94 \times 1.87 + 0.41 \times 1.44 \approx 6.1 .
\]

Two lessons, numerically. \emph{Saturation:} document B has four times document A's
count of ``transformer,'' yet its TF component rises only from $1.76$ to $1.87$---both
are already pressing against the $k_1 + 1 = 2.2$ asymptote, so piling on occurrences
buys almost nothing. \emph{Length normalization:} B's higher raw counts are further
taxed by its twice-average length. Under raw TF--IDF the scores would have been
$\approx 9.6$ vs.\ $\approx 36.8$---B looks nearly four times as relevant; under BM25
the gap is $5.8$ vs.\ $6.1$, about $5\%$. That is exactly the behavior a ranking
function should have: more mentions are better evidence, but only slightly, and
padding a document with repeated keywords does not pay.
...
\subsection{Inverted Index Mechanics: Why BM25 Is Fast}
\label{sec:inverted_index}

Claiming that BM25 handles ``millions of documents in milliseconds'' hand-waves the
actual mechanism, and staff-level retrieval interviews probe exactly this layer. The
answer is the \textbf{inverted index}---the data structure at the heart of Lucene,
Elasticsearch, and every classical search engine---plus query-evaluation algorithms
that avoid scoring most documents at all.

\paragraph{Postings lists.}
An inverted index maps each vocabulary term to a \textbf{postings list}: the sorted
list of IDs of documents containing that term, stored with the per-document term
frequency (and, for phrase queries, token positions). The term dictionary also stores
$\text{df}(t)$, and a per-document table stores lengths $|d|$---every quantity in
Eq.~\eqref{eq:bm25} is therefore a precomputed lookup. Crucially, scoring a query
touches only the postings lists of the query's terms, never the full collection: for a
two-term query over 10M documents, the work is proportional to the length of two
postings lists (perhaps tens of thousands of entries), not to 10M documents. Postings
are delta-encoded and compressed, and skip pointers allow jumping ahead without
decompressing everything.

\paragraph{Index-time vs.\ query-time analysis.}
The \emph{analyzer chain} (tokenization, lowercasing, stemming, stopword handling) runs
twice: on documents when the index is built, and on the query when it is issued---and
the two must agree, or matching silently breaks (a stemmed index queried with unstemmed
terms loses recall with no error message). Expansion work can be placed on either side:
index-time expansion (e.g., adding synonyms into postings) makes the index larger but
queries fast; query-time expansion keeps the index lean but adds per-query latency, and
can be changed without reindexing. ``Where in the chain does this transformation run,
and what does moving it cost?'' is a classic search-infrastructure interview probe.

\paragraph{Query evaluation: TAAT vs.\ DAAT.}
\textbf{Term-at-a-time (TAAT)} evaluation processes one postings list in full,
accumulating partial scores per document in a hash table, then moves to the next term.
\textbf{Document-at-a-time (DAAT)} evaluation instead advances a cursor over each
query term's postings in parallel (lists are doc-ID-sorted), fully scoring one document
before moving on. DAAT needs no large accumulator structure and---because each
document's complete score is known the moment it is visited---enables early
termination for top-$k$ retrieval, which is why production engines are DAAT.

\paragraph{Top-$k$ pruning: WAND and block-max WAND.}
Retrieval needs the top $k$ documents, not scores for all matches. \textbf{WAND}
stores, for each term, an upper bound on the score contribution that term can ever
make; during DAAT evaluation, any document whose summed upper bounds cannot beat the
current $k$-th best score is skipped without being scored---the cursors leap over it.
\textbf{Block-max WAND} sharpens this by storing upper bounds per compressed block of
postings rather than per term, letting the engine skip entire blocks whose local
maxima are too small. In practice these methods score only a small fraction of the
matching documents while returning exactly the same top $k$---that, plus compressed
postings that live comfortably in memory, is the honest content behind ``millions of
documents in milliseconds.'' The systems treatment---compression schemes, index
tuning, learned sparse retrieval on the same index---is in Volume~III [SR 2]; here the
goal is that you can explain the mechanism, not build the engine.
...
The Penn Treebank tagset defines 45 tags for English (36 word-class POS tags plus
punctuation and symbol tags), including:
...
\begin{tikzpicture}[
    ...
    % Entity spans (braces sit below the tag rows, not across them)
    \draw[thick, deepblue, decorate, decoration={brace, amplitude=4pt, mirror}]
        ([yshift=-1.8cm]w0.south west) -- ([yshift=-1.8cm]w1.south east)
        node[midway, below=9pt, font=\scriptsize\sffamily, text=deepblue] {PER};
    \draw[thick, deepgreen, decorate, decoration={brace, amplitude=4pt, mirror}]
        ([yshift=-1.8cm]w3.south west) -- ([yshift=-1.8cm]w5.south east)
        node[midway, below=9pt, font=\scriptsize\sffamily, text=deepgreen] {LOC};
\end{tikzpicture}
...
single-token entities. Empirically, BIOES improved NER F1 by 0.5--1.5\% over BIO in the
CRF-era literature (Ratinov \& Roth, 2009); with modern transformer-based taggers the
difference is usually within run-to-run noise, so BIO remains a perfectly defensible
default.
\end{keyinsight}
...
\begin{tikzpicture}[
    ...
    label/.style={align=center, font=\scriptsize\sffamily\itshape, text=deepgreen, fill=white, inner sep=1pt}
]
    % Words
...
\label{sec:ngram_forward}
% ============================================================================

Language models assign probabilities to sequences of words, and n-gram models are the
classical approach. This material---the chain rule decomposition, the Markov assumption,
maximum likelihood estimation from counts, smoothing techniques (Laplace, add-$k$,
interpolation, Kneser-Ney), and perplexity---forms the probabilistic backbone of
statistical NLP and is covered in full depth, with derivations and worked examples, in
Chapter~\ref{chap:statistical_nlp}: the models in Section~\ref{sec:ngram_lm}, the
evaluation metric in Section~\ref{sec:perplexity}. The feature-side use of the same
$n$-gram counts is Section~\ref{sec:ngrams} above.

For this chapter's narrative, the essential point is that n-gram models formalized the
insight that local word context carries predictive signal, and their limitations (data
sparsity, no generalization across semantically similar words) motivated the neural
...
(1) latency requirements are extremely tight ($< 1$ ms); (2) the task is simple (binary
classification with clear keyword signals); (3) interpretability is required (TF-IDF
weights are directly inspectable). The once-standard claim that classical models also win
whenever training data is very small ($< 100$ examples) is now contested: SetFit-style
contrastive fine-tuning and LLM few-shot prompting are usually stronger in that regime, so
treat small data as a reason to \emph{try} the classical baseline, not to assume it wins. A
...
explicit boundary information; in CRF-era systems this typically improved F1 by
0.5--1.5\%, though with transformer taggers the gap is usually negligible.
...
% --- Question 9: Naive Bayes vs. BERT ---
\begin{interviewq}
...
parameters to estimate. With 50 labeled examples, vanilla BERT fine-tuning is prone to
overfitting on the classification head, while Naive Bayes with TF-IDF features can still
produce reasonable results. The honest caveat: plain fine-tuning is no longer the strongest
neural option in this regime---SetFit-style contrastive fine-tuning and LLM few-shot
prompting typically beat both---so this is an argument for Naive Bayes as a fast baseline,
...
% --- Question 10: Classical-ML filtering for a pretraining corpus ---
\begin{interviewq}
\textbf{Q: Design the classical-ML filtering stage for a 10T-token pretraining corpus:
quality filtering, deduplication, and benchmark decontamination.}

\badgeLsix{L6} \quad \badgetype{System Design} \quad \badgefreq{\freqmed}

\stronganswer
I would structure the pipeline cheapest-first, so each stage shrinks the corpus before
the next, more expensive one runs. Everything below is CPU-only, embarrassingly
parallel, and built from classical NLP components (Section~\ref{sec:classical_llm_stack}).

\textbf{Stage 1 -- Heuristic and language filtering.} URL/domain blocklists,
boilerplate stripping, and document-level heuristics (length bounds, symbol-to-word
ratio, repetition ratios---C4/Gopher-style rules). Language identification with a
fastText LangID model routes documents into per-language pipelines. These reject the
bulk of raw crawl at negligible cost.

\textbf{Stage 2 -- Model-based quality filtering.} Two complementary classical
scorers. (a) A \emph{KenLM perplexity filter}, CCNet-style: score each document with a
Kneser-Ney 5-gram model trained on a clean reference corpus (Wikipedia) and drop or
downweight the high-perplexity tail---gibberish, boilerplate, and spam score badly
against a clean-text n-gram model. (b) A \emph{fastText quality classifier},
DCLM/FineWeb-Edu-style: train a linear classifier with positives from
known-high-quality text (instruction data, or pages graded by an LLM) and negatives
sampled from raw crawl, and keep the top-scoring fraction. Throughput is the design
constraint that dictates these choices: a fastText model classifies hundreds of
thousands of documents per second per CPU core, so 10T tokens is a modest CPU-cluster
job, whereas pushing 10T tokens through even a 1B-parameter neural scorer is on the
order of $10^{22}$ FLOPs---a visible fraction of the pretraining compute itself. The
LLM is used only where it is cheap: labeling $10^5$--$10^6$ classifier training
examples, never scoring the corpus.

\textbf{Stage 3 -- Deduplication.} Exact duplicates first, via document hashing. Then
near-duplicates via \emph{MinHash over n-gram shingles}: represent each document as its
set of word 5-grams, compute a MinHash signature (on the order of 100 hash functions),
band the signatures with LSH so that similar documents collide, and drop documents
whose estimated Jaccard similarity to a retained one exceeds a threshold
($\approx 0.8$). This controls memorization, saves compute otherwise spent re-reading
the same page thousands of times, and removes one vector of eval contamination.
Suffix-array substring dedup is the heavier alternative when repeated passages inside
otherwise-distinct documents matter.

\textbf{Stage 4 -- Benchmark decontamination.} Scan the surviving corpus for long
n-gram collisions (the GPT-3 report used 13-grams) against every evaluation set the
model will be reported on, and remove or flag the hits. This runs \emph{after} dedup
(so contaminated duplicates cannot slip back in) and must be re-run whenever a new
benchmark is adopted.

\textbf{Validation.} Filter thresholds are hyperparameters, so hold them accountable
the DCLM way: small-scale proxy pretraining runs on candidate corpora, compared on
downstream evals---not by eyeballing filtered samples.

\redflags
\begin{itemize}
    \item Proposing to score all 10T tokens with an LLM without doing the cost
          arithmetic.
    \item Knowing only exact-match dedup---not MinHash/Jaccard over shingles, or why
          near-duplicate detection is the hard part.
    \item Omitting decontamination entirely, or running it before deduplication.
    \item No validation story: treating filter thresholds as self-evidently correct.
\end{itemize}

\interviewtest{Whether the candidate can apply classical NLP machinery to the most
consequential data-engineering problem in modern ML, reason about throughput and cost
at corpus scale, and sequence a multi-stage pipeline sensibly.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5}: Names the stages (heuristics, quality filtering, dedup,
          decontamination) with the right classical tool for each.
    \item \badgeLsix{L6}: Gets the ordering and cost reasoning right
          (cheap-to-expensive, CPU throughput vs.\ neural-scorer FLOPs), cites the
          concrete pattern instances (CCNet, DCLM/FineWeb-Edu, 13-gram
          decontamination), and proposes proxy-run validation.
    \item \badgeLseven{L7}: Discusses the open judgment calls: how aggressive dedup
          should be (some repetition of high-quality text helps), per-language
          threshold calibration, the interaction of filtering with data mixtures and
          curricula, and contamination that 13-gram overlap misses (paraphrased or
          translated test items). Points to the end-to-end treatment in the Deep
          Learning volume's \emph{Pretraining at Scale} chapter [DL].
\end{itemize}

\followups{How do you choose the Jaccard threshold and shingle size? When is
suffix-array substring dedup worth its cost? Why might you deliberately \emph{keep}
some duplicates? How would you detect paraphrase-level benchmark contamination that
