...
\begin{itemize}
    \item \textbf{Modern default:} Apply LoRA to \emph{all} linear layers---the
          attention projections ($\mW_Q$, $\mW_K$, $\mW_V$, $\mW_O$) \emph{and}
          the FFN projections ($\mW_1$, $\mW_2$, and gate projection for
          SwiGLU).  This is the default in the current PEFT ecosystem.
    \item \textbf{Historical minimal config:} The original LoRA paper (2021)
          adapted only the query ($\mW_Q$) and value ($\mW_V$) projections.
          Q+V-only saves parameters and can suffice for tasks close to the
          pre-training distribution, but it is the paper's minimal
          configuration, not current best practice.
    \item \textbf{Empirical finding:} The QLoRA ablations (Dettmers et al.,
          2023) showed that adapting all linear layers---FFN included---is
          what closes the gap to full fine-tuning; attention-only
          configurations underperform, especially when the task requires
          significant knowledge adaptation (e.g., domain-specific fine-tuning).
\end{itemize}
...
\paragraph{Quality.}
QLoRA quality is \emph{configuration-dependent}, not a fixed quantization
penalty.  With the paper's full recipe---NF4 with double quantization and LoRA
adapters on \emph{all} linear layers---Dettmers et al.\ report matching 16-bit
LoRA and 16-bit full fine-tuning; this parity claim is the paper's headline
result (Guanaco).  Gaps of a few percent do appear with weaker configurations:
low rank, adapters restricted to a subset of modules (e.g., attention-only),
or harder domain shifts.  State the parity result first, then the caveats.
...
QLoRA         & LoRA on 4-bit base model    & 0.1--1\%    & 90--100\% & None (merge) \\
...
the typical fraction of full fine-tuning performance retained.  QLoRA reaches
the top of its range (parity with 16-bit LoRA/full FT) when configured with
all-linear-layer NF4 adapters (Dettmers et al., 2023); weaker configurations
land lower.}
