...
catastrophic forgetting mitigation, and how to choose an adaptation method---full
fine-tuning, LoRA, QLoRA, or the additive PEFT family (per-method mechanics: [DL~16]).
...
\[
    ...
    \Bigr]
\]
where $y_t = \mathds{1}[\tilde{x}_t = x_t]$ indicates whether position $t$ contains
...
Every stage of the adaptation lifecycle---continued pre-training, SFT,
preference tuning---can be run full-weight or with only a small set of added
parameters trained on top of a frozen base.  That second option,
\textbf{parameter-efficient fine-tuning (PEFT)}, recovers most of full
fine-tuning's quality at a fraction of its memory and storage cost.  PEFT is
not a stage of its own but a knob on every stage; this section sets the knob.

\subsection{What LoRA Is}
\label{sec:lora}

LoRA (Hu et al., ICLR 2022) is the default PEFT method.  It freezes each
pre-trained weight matrix $\mW_0 \in \R^{d \times k}$ and learns the
\emph{update} in factored form:
\[
    \mW = \mW_0 + \frac{\alpha}{r}\,\mB\mA,
    \qquad
    \mB \in \R^{d \times r},\;\; \mA \in \R^{r \times k},\;\; r \ll \min(d, k),
\]
so a rank-16 adapter on a 7B model trains on the order of $0.1\%$ of the
parameters.  The justification is the \textbf{intrinsic dimensionality} result
(Aghajanyan et al., 2021): the directions that matter when adapting a
pre-trained model span a subspace far smaller than the parameter space, so a
low-rank $\Delta\mW$ gives up little.  Two consequences drive the lifecycle
decisions that follow.  The base weights are never touched, so one frozen copy
backs many few-MB task adapters instead of a full model copy per task.  And
because $\mB\mA$ is only a weight delta, it can be \textbf{merged} into
$\mW_0$ after training, leaving the original architecture and \emph{zero}
added inference latency---the property separating LoRA from the additive
methods below.

\paragraph{Quantized and refined variants.}
\label{sec:qlora}
\label{sec:dora}
QLoRA (Dettmers et al., 2023) holds the frozen base in 4-bit NormalFloat and
trains 16-bit adapters on top---this is what puts a 65B fine-tune on one
48\,GB GPU, and configured well it matches 16-bit LoRA rather than paying a
fixed quantization penalty.  DoRA (2024) and the rest of the LoRA lineage
refine how the update is parameterized without changing the picture above.
...
\begin{tabularx}{\textwidth}{@{}l X X@{}}
\toprule
\textbf{Method} & \textbf{Choose it when} & \textbf{What it costs} \\
\midrule
Full fine-tuning & $>$100K examples, or a real distribution shift (new
  language or domain); maximum quality bar &
  Multi-GPU memory; a model copy per task; forgetting must be managed
  (Section~\ref{sec:catastrophic-forgetting}) \\
LoRA & Default for $10^3$--$10^5$ examples on a 7--13B base; many tasks over
  one shared base &
  Small gap on far-from-pretraining tasks unless rank and target modules rise \\
QLoRA & The base does not fit unquantized (33--70B on one GPU) &
  Slower steps; merging needs a dequantize--merge--requantize trip \\
Adapters & Multi-task serving with per-task modules kept separate by design &
  Cannot merge: extra matmuls at every layer, permanently \\
Prefix / prompt tuning & Very large base ($\gtrsim$10B), light task, smallest
  per-task artifact &
  Weakest below $\sim$10B; eats context positions, adds per-pass cost \\
IA3 & Few-shot adaptation where even LoRA's budget is too much &
  Least expressive; rescaling cannot express arbitrary weight updates \\
\bottomrule
\end{tabularx}
\caption{Choosing an adaptation method from the lifecycle perspective.
Per-method mechanics and quality numbers are in [DL~16].}
\label{tab:peft-comparison}
\end{table}

\paragraph{The rest of the vocabulary.}
\textbf{Prefix tuning}\label{sec:prefix-tuning} (2021) prepends learnable
key/value vectors at every layer; \textbf{prompt
tuning}\label{sec:prompt-tuning} (2021) prepends soft tokens at the input
embedding only; \textbf{adapters}\label{sec:adapters} (2019) insert bottleneck
MLPs between sublayers; \textbf{IA3}\label{sec:ia3} (2022) learns element-wise
scaling vectors for keys, values, and FFN activations.  All four are
additive---none folds back into the base weights, which is why LoRA won.

\begin{keyinsight}[Where the PEFT Mechanics Live]
The mechanics are canonical in \textbf{[DL~16]}: zero-initialization of $\mB$,
choosing rank $r$ and the $\alpha/r$ scaling, which modules to target (and why
all-linear beats attention-only), merging and un-merging, QLoRA's NF4 /
double-quantization / paged-optimizer internals, the DoRA and rsLoRA lineage,
the full method comparison, and serving many adapters behind one base model.
\end{keyinsight}
...
pre-training and alignment (RLHF/DPO, covered in Chapter~\ref{chap:rlhf_alignment}).
...
whose responses were, in human evaluations, preferred to or tied with
DaVinci-003 and Bard in a majority of comparisons---though GPT-4 was still
preferred over LIMA 57\% of the time.  The
...
\begin{keyinsight}[Which Adaptation Questions Live Where]
This bank asks the \emph{adaptation decisions} this chapter owns: pre-train,
continue pre-training, fine-tune, or prompt; full fine-tuning or PEFT at a given
data and compute budget; and how the choice moves through the pre-training
$\to$ SFT $\to$ preference-tuning lifecycle.  The \emph{PEFT mechanics}
questions are canonical in \textbf{[DL~16]} and are deliberately not repeated
here: ``explain LoRA---why does it work, and how do you pick rank and
$\alpha$,'' ``what is the low-rank assumption and when does it break,''
``adapters vs.\ LoRA vs.\ prefix tuning---architectural differences and when to
use each,'' ``estimate the trainable parameters for rank-16 LoRA on a 7B
model,'' ``walk through QLoRA (NF4, double quantization, paged optimizers) and
its memory budget,'' and ``serve 100 LoRA adapters from one base model.''
Expect an interviewer to cross the line in both directions; answer the decision
here and hand off the knobs to [DL~16].
\end{keyinsight}
...
(4) You need fast iteration (LoRA's dominant saving is \emph{memory}---far
smaller optimizer states enable larger batches on the same hardware; the
wall-clock speedup itself is a more modest $\sim$1.2--1.5$\times$, since
activations and the full backward pass through the frozen network remain).
...
\textbf{Where the mechanics live:} rank and $\alpha$ selection, which modules to
target, merging, and QLoRA's internals are canonical in [DL~16].  This question
is the budget-and-data decision, not the knobs---an answer that opens with the
LoRA formula and never reaches ``how much data, how much GPU, how many tasks on
one base'' has missed what is being asked.
...
\followups{What if you need to add a new language to the model?  How would you
decide between LoRA and continued pre-training?  (Rank selection and the
LoRA/quantization interaction are [DL~16] territory.)}
\end{interviewq}
...
(1)~\textbf{Start with LoRA} on a strong base model (e.g., LLaMA-2 7B or 13B).
At this data scale the argument is regularization, not memory: a small adapter
has too little capacity to memorize 1,000 examples the way a full fine-tune
would.  (Rank, $\alpha$, target modules, and the LoRA learning-rate scale are
[DL~16]; so is the 10K-example pipeline with its wall-clock and cost anchors.)
...
demonstrated this).
(3)~\textbf{Train for 3--5 epochs} with early stopping based on validation loss.
(4)~\textbf{Data augmentation:} use the base model itself to generate synthetic
examples (bootstrap from the 1,000), filter by quality, and add to training.
(5)~\textbf{Evaluation:} hold out 10--15\% for validation; use task-specific
metrics, not just loss.

If quality is still insufficient: (6)~continued pre-training on domain-specific
unlabeled text (before SFT) to adapt the model's representations to the domain.
(7)~Few-shot prompting as a baseline---for some tasks, a well-prompted LLM with
