...
\end{itemize}
BIOES is slightly more expressive than BIO: every entity must be explicitly closed by an
\texttt{E-} tag (or be a single-token \texttt{S-} entity), which gives the model direct
supervision of entity boundaries and separates adjacent entities of the same type more
cleanly.  For example, the two adjacent locations in ``New York London'' are tagged
\texttt{B-LOC E-LOC S-LOC}---the \texttt{E-LOC}/\texttt{S-LOC} transition marks the split
into two entities explicitly.
...
where $T$ is the sequence length and $K$ is the number of labels.  On top of BERT, however,
the gains are modest: published results range from no measurable improvement to roughly half
an F1 point, with the clearest benefit on small training sets and on multi-token entities
where invalid tag transitions are otherwise common.  Note that a linear-chain CRF cannot
represent \emph{nested} entities at all---nested NER requires span-based models, discussed
under NER challenges below.
\end{keyinsight}
...
\begin{tikzpicture}[
    ...
    lbl/.style={align=center, font=\scriptsize\sffamily, text=gray!70!black}
]
...
Modern open-domain QA systems are essentially RAG systems; see
Chapter~\ref{chap:rag} for the full treatment of retrieval-augmented
generation.  The key design decisions are: retrieval method (sparse
...
\begin{itemize}
    \item \textbf{mBART (Liu et al., 2020):} Denoising autoencoder pretrained on monolingual
          data in 25 languages, then fine-tuned on parallel data for a specific language
          pair.  Zero-shot translation between pairs unseen in fine-tuning is a property of
          \emph{many-to-many} multilingual training (as in mBART-50 and M2M-100), not of the
          original per-pair mBART recipe.
...
% User input/output, stacked above the pipeline so the row fits the text block
\node[databox, above=0.7cm of nlu] (user) {User utterance};
\node[databox, above=0.7cm of nlg] (resp) {System response};
...
% Backend query/response, routed either side of the action annotation
\node[box, below=1.9cm of policy, fill=lightyellow, draw=orange!60, minimum width=2.6cm] (db) {Backend/DB};
\draw[arr, orange!60] ([xshift=-1.15cm]policy.south) -- ([xshift=-1.15cm]db.north);
\draw[arr, orange!60] ([xshift=1.15cm]db.north) -- ([xshift=1.15cm]policy.south);
\end{tikzpicture}
...
trivial infrastructure cost.  Now price the LLM alternative honestly, using illustrative
API rates (2026-era anchors: small/mid-tier models around $c_{\text{in}} \approx
\$0.25\text{--}3$ per million input tokens; frontier models roughly \$3--15/M input and
\$15--75/M output).  Classifying $10^9$ messages/day at ${\sim}500$ tokens each consumes
$5 \times 10^{11}$ input tokens/day, so even the cheap tier costs
$500{,}000\,\text{M tokens} \times c_{\text{in}} \approx \$125\text{K}\text{--}\$1.5\text{M}$
\emph{per day}---versus effectively free CPU inference---with no meaningful accuracy
improvement on this well-defined binary task.
