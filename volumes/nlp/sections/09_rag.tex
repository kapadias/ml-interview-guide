...
\begin{itemize}
    ...
    \item \textbf{Efficiency:} Inverted index lookup is $O(1)$ per term
          (scoring then costs time proportional to the length of each term's
          posting list); no GPU required at query time.
...
per-token embeddings for every document, and the index-size penalty is
\emph{compression-dependent}: stored uncompressed, per-token embeddings can be
roughly two orders of magnitude larger than a single-vector bi-encoder index,
while ColBERTv2's residual compression brings the overhead down to roughly
$6$--$10\times$.  Quote the assumption along with the number.
...
token-level interaction.  However, each (query, document) pair requires its
own full transformer forward pass---$O(N)$ forward passes to score $N$
candidate documents, with nothing precomputable offline.  This makes
