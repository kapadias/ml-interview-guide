...
compression, citation, and what regulated domains add on top), RAG
evaluation, and the failure taxonomy that tells you which layer broke.  The
retrieval \emph{infrastructure} underneath it is Volume~III: lexical
retrieval systems are [SR 2], vector-index internals and sizing are [SR 3],
bi-encoders, rerankers and hybrid fusion are [SR 4], and production
retrieval---freshness, serving, caching, monitoring, and RAG's retrieval
stage---is [SR 8]; the long-context-versus-retrieval architectural
...
% Connection from vector DB to retrieval (routed below both rows of boxes)
\draw[darr] (vecdb.south) -- ++(0,-0.9) -| (retrieve.north);

% Separator line between the offline and online phases
\draw[gray!40, dashed, thick] ([xshift=-0.6cm, yshift=-0.25cm]docs.south west) --
    ([xshift=0.6cm, yshift=-0.25cm]vecdb.south east);
...
\subsection{Retrieval in Regulated and High-Stakes Domains}
\label{sec:high-stakes-rag}

Legal research, clinical decision support, financial disclosure, and
compliance Q\&A run the same pipeline as a support bot, but under a different
cost asymmetry: a wrong answer is a liability rather than a bad experience,
and a confidently cited wrong answer is worse than no answer at all.  Five
things change, and none of them is the model.

\paragraph{Authority-aware indexing and the citation graph.}
In these corpora documents are not independent.  An opinion cites,
distinguishes, or is overruled by another; a regulation supersedes a prior
version; a contract clause incorporates a schedule by reference; a clinical
guideline is withdrawn and replaced.  Embedding similarity ranks a passage
that \emph{reads} relevant---it has no idea the case was overruled last year.
Index the citation and reference graph alongside the vectors and use it twice:
as a ranking prior, so an authoritative, still-good-law, heavily-cited source
outranks an equally similar secondary summary, and as an expansion step, so
retrieving a clause also pulls the definitions and schedules it incorporates.
Retrieving superseded text and citing it fluently is the characteristic
high-stakes failure, and it is a retrieval defect, not a generation one.  The
validity metadata this depends on---effective dates, supersedes-links,
deprecation flags carried in the index---is [SR 8].

\paragraph{Jurisdiction and effective date are pre-filters, not ranking
features.}
A statement true in Delaware and false in California, or true under the 2023
rule and false under its 2025 replacement, is not \emph{less relevant}---it is
wrong, and a ranker that merely down-weights it will still surface it when the
right document is missing.  Carry jurisdiction, an effective-date interval
(valid-from, valid-to), and document version as first-class metadata, and
pre-filter the candidate set on them.  Resolve the as-of date from the matter
rather than from the wall clock: the question is usually what the rule said on
the date of the transaction, not what it says today.

\paragraph{Access control travels with retrieval, never after generation.}
Attorney--client privilege, ethical walls between teams on opposing sides of a
matter, and minimum-necessary rules for patient data are per-user, per-matter
predicates on the candidate set.  Section~\ref{sec:vector-db} makes the
mechanical point---ACLs must be pre-filters, because anything retrieved can
leak into the generated answer---and the high-stakes setting adds two
requirements.  Authorize each request against the caller's live entitlements
rather than an ACL snapshot frozen into the index at ingest time, because
entitlements change faster than reindexing does.  And treat the caches as part
of the security boundary: a semantic or answer cache keyed on query text alone
will happily serve one user's privileged answer to another ([SR 8]).
Filtering the citations out of a finished answer is not a control---the
passage has already shaped the text.

\paragraph{Provenance and audit logging.}
You must be able to reconstruct, months later, why the system said what it
said: the index version and embedding model, the retrieved candidates and
their order, what survived reranking, the assembled prompt, the model version,
and the answer with its citations.  Log the retrieval set, not just the
answer---an answer alone can be neither diagnosed nor defended.  This is also
what makes a citation verifiable rather than decorative: each claim maps to a
chunk, each chunk to a document identifier, section, and version, so a
reviewer can open the source \emph{at the version the system read}.  The log
is itself a compliance object, with its own retention policy and its own
access controls.

\paragraph{A higher grounding bar, and abstention as a first-class output.}
Faithfulness thresholds that are fine for a support bot are not fine when a
professional will rely on the answer.  Concretely: require a citation for
every load-bearing claim and verify it mechanically---NLI entailment or
citation checking (Section~\ref{sec:rag-evaluation})---rather than trusting
the model's own brackets; make ``the retrieved sources do not settle this'' a
supported, well-tested output rather than a failure path, and accept a lower
answer rate to get a higher precision; and route to human review by risk
class, not by model confidence alone.  Score the evaluation asymmetrically
too: if an unsupported confident answer costs the same as an abstention, the
metric rewards exactly the behavior you are trying to eliminate.

None of this is a different RAG architecture.  It is the same pipeline with
metadata, filter order, logging, and abstention policy promoted from polish to
requirements---which is precisely what an interviewer in these domains probes.
...
\begin{itemize}
    ...
    \item \badgeLseven{L7} Designs for multi-turn conversation (session context),
          ...
          [SR 8].  If the deployment is regulated rather than consumer support,
          carries the extra requirements of Section~\ref{sec:high-stakes-rag}:
          jurisdiction and effective-date pre-filters, entitlement-time ACLs
          enforced inside retrieval rather than after generation, an auditable
          retrieval log, and a tuned abstention path
\end{itemize}
...
What would change if this knowledge base were a law firm's matter files rather
than public support articles?}
\end{interviewq}
...
%% QUESTION 10 %%
\begin{interviewq}
\textbf{Q: Your RAG system answers factual lookup questions well but fails on
questions that require combining facts across documents---``which customers on
the legacy plan have an open P1 ticket?'' or ``did the Q3 pricing change
contradict the renewal terms?''  Retrieval looks healthy: every returned chunk
is on topic. Diagnose it.}

\badgeLsix{L6} \quad \badgetype{Debugging} \quad \badgefreq{\freqhigh}

\stronganswer
The symptom looks like a reasoning failure, so the instinct is to blame the
generator.  It is almost always an \emph{evidence-set} failure, and the
differential is what separates the two.

\textbf{Work the differential first.}
(1)~\textbf{Retrieval returns locally relevant chunks, not a sufficient set.}
Each chunk is individually similar to the query, which is exactly what the
retriever optimizes; but the answer lives in facts spread across several
documents, and nothing in the pipeline scores a \emph{set} for sufficiency.
``All $k$ chunks are relevant'' and ``the $k$ chunks contain the answer'' are
different statements, and recall@$K$ measured as ``did any relevant chunk
appear'' will report the system as healthy while it is not.
(2)~\textbf{Chunk-level similarity cannot express a multi-hop query.}  A
composed question embeds to roughly the average of its parts, which lands near
documents that discuss both topics generically rather than near the two
specific chunks that each answer one part---and for a genuine second hop the
bridging entity is not in the query at all (``the film that won the Palme d'Or''
must be resolved to a title before the director can be retrieved), so no
single-shot query, at any $k$, can reach it.  Aggregation queries are worse
still: ``which customers'' asks for a set defined by a predicate, and there is
no chunk whose text is that set.
(3)~\textbf{The generator cannot synthesize what was never retrieved.}  Given a
partial evidence set, a model either hedges or fills the gap from parametric
memory---which is why the failure surfaces as confident, plausible, wrong
prose, and why upgrading the model moves the number very little.

\textbf{Confirm it before fixing it.}  Build a small labelled slice of
reasoning queries and annotate the \emph{full} evidence set for each---every
chunk needed, not one gold chunk.  Then measure all-hops recall (did retrieval
return \emph{all} required chunks?) alongside the usual recall@$K$; the gap
between the two curves is the diagnosis.  Confirm with an oracle-context test:
hand the generator the gold chunks and re-ask.  If it now answers, the defect
is retrieval completeness; if it still fails, you have a real synthesis limit
and the fixes below will not help.  This is the layered decomposition of
Section~\ref{sec:rag-evaluation} applied to a failure mode that hides between
its layers.

\textbf{Fixes, cheapest first.}
(1)~\textbf{Query decomposition.}  One LLM call before retrieval splits the
question into sub-queries, each retrieved independently, with the union
assembled for synthesis (Section~\ref{sec:advanced-rag}).  This is the right
first move when the parts are \emph{independent}---comparisons, multi-entity
lookups, ``and'' questions---and it costs one extra call plus $N$ retrievals.
(2)~\textbf{Iterative (multi-hop) retrieval.}  When hop~2 depends on hop~1's
answer, decomposition cannot help: the loop must retrieve, read, reformulate,
and retrieve again.  Budget it explicitly---a hop cap, a termination
condition, and a per-query spend ceiling---because $N$ hops is $N$ retrievals
and $N{+}1$ generations, which turns a 300\,ms query into seconds.  The
agentic form, where retrieval is a tool the model may invoke zero or more
times and the loop decides when it has enough, is [DL 27]; what an agent loop
does to the retrieval tier's QPS and tail latency is [SR 8].
(3)~\textbf{Higher $K$ with a reranker.}  Raising first-stage $K$ so that all
hops sit in the candidate pool, then reranking down to a clean context, is the
cheapest infrastructure change and sometimes enough---but only for queries
whose evidence was reachable by the original query in the first place.  It
does nothing for a second hop whose entity is still unknown, and it is not
free: more chunks in context is more distraction and more exposure to
lost-in-the-middle (Section~\ref{sec:rag-vs-ft}), which is precisely why the
reranker, not the raw $K$, is the load-bearing part
(Section~\ref{sec:reranking}).
(4)~\textbf{Aggregation-friendly chunking and indexing.}  If the question is
about a customer, the retrievable unit should be a customer record, not the
paragraph that happens to mention one.  Parent-child retrieval, document-level
summaries indexed alongside chunks, and entity-level or table-level records
give the retriever something whose embedding actually denotes the thing being
asked about (Section~\ref{sec:chunking}).  This is a reindex, so it is a
sprint, not an afternoon.
(5)~\textbf{Admit when it is not a RAG query.}  ``Which customers on the
legacy plan have an open P1 ticket'' is a join with a filter over two systems
of record.  A text-to-SQL path or a graph traversal answers it exactly, in
milliseconds, with no chance of a missed chunk; no amount of retrieval tuning
will make similarity search do it correctly, because the query is set-valued
and the index is not.  The mature answer routes by query class---a light
classifier or planner sends set-valued, aggregate, and comparative queries to
the structured path and keeps lookups on the vector path, with retrieval as
one tool among several.  Teams that miss this spend a quarter rebuilding a
worse database out of embeddings.

Finally, hold the reasoning slice out as its own evaluation segment
permanently.  Averaged into one headline number it disappears, and every fix
above trades some cost or latency on the factual slice to buy it back.

\redflags
\begin{itemize}
    \item Jumps to ``use a stronger model'' or prompt tweaks when the required
          facts were never in the context
    \item Reports recall@$K$ (``a relevant chunk was retrieved'') as evidence
          that retrieval is fine, without ever asking whether the set was
          \emph{sufficient}
    \item Raises $k$ with no reranker, adding distractors and lost-in-the-middle
          exposure while leaving dependent hops unreachable
    \item Reaches straight for an agentic multi-hop loop without costing its
          latency and spend
    \item Never considers that set-valued or aggregate queries belong to a
          database or a knowledge graph rather than to RAG
\end{itemize}

\interviewtest{Whether the candidate can tell a retrieval-completeness failure
from a generation failure when the symptom impersonates poor reasoning, and
whether they will scope a query class out of RAG instead of tuning the
retriever forever.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Recognizes that the needed facts are spread across
          documents and proposes query decomposition or multi-hop retrieval
    \item \badgeLsix{L6} Works the differential explicitly, proposes the
          oracle-context test and an all-hops recall metric on a reasoning
          slice, and orders the fixes by cost rather than listing them
    \item \badgeLseven{L7} Routes by query class with a planner, budgets the
          agentic loop (hop cap, termination, per-query spend) and prices its
          serving impact, proposes entity- or graph-shaped indexes for
          set-valued questions, and wires the reasoning slice into continuous
          evaluation so the fix cannot silently regress the factual slice
\end{itemize}

\followups{How would you build a labelled evaluation set for multi-hop queries
without hand-annotating every hop?  What termination condition would you give
an iterative retrieval loop, and what does one cost per query?  At what point
do you tell the team a query class needs a knowledge graph or a SQL layer
rather than a better retriever?}
\end{interviewq}
...
\begin{enumerate}
    ...
    \item \textbf{Advanced patterns}---HyDE, query decomposition, multi-hop
          ...
          the limitations of basic retrieve-then-generate.  Regulated and
          high-stakes deployments add a further set: authority-aware and
          citation-graph indexing, jurisdiction and effective-date
          pre-filtering, entitlement-time access control inside retrieval,
          provenance logging, and abstention as a first-class output.
