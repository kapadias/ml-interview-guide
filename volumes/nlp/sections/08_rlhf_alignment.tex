...
% process reward models, the 2025-26 RLVR/GRPO recipe in practice, and
% the practical challenges of aligning LLMs.
% ============================================================================
...
optimization with KL constraints, DPO as a simpler alternative,
Constitutional AI for scalable oversight, and the 2025--26 shift to
verifiable rewards and GRPO that now carries reasoning training
(Section~\ref{sec:rlvr_practice}).  Understanding RLHF is essential
...
difficulty, safety), and filtering for low-quality annotations.
InstructGPT-era datasets ranged from 50K to 500K preference pairs; by 2026,
frontier pipelines train reward models on \emph{millions} of pairs, the
majority synthetic---model-generated responses ranked by AI judges
(Section~\ref{sec:cai})---built around a smaller human-labeled core.
...
\item \textbf{Evaluation:} \textbf{RewardBench} (2024) is the standard
  public reference point for evaluating reward models (held-out
  preference accuracy across chat, safety, and reasoning categories),
  though a high benchmark score does not guarantee robustness to hacking
  under RL optimization pressure.
\end{itemize}
...
Interviewers expect numbers here, and the chapter's qualitative story has
concrete anchors.  The KL coefficient $\beta$ is typically
\textbf{0.01--0.2} for RM-based RLHF; many implementations use an adaptive
controller that adjusts $\beta$ to hold a target KL rather than fixing it.
The realized \textbf{KL budget is small}: well-tuned runs keep the policy
within \textbf{single-digit nats per sequence} of the reference model---a
few nats is common, and a run drifting to tens of nats per sequence is
usually already reward hacking (Section~\ref{sec:reward_hacking}).
Compute-wise, the RL stage typically costs \textbf{3--5$\times$ the SFT
stage}---dominated by rollout generation, not gradient steps
(Section~\ref{sec:rlhf_infra})---while remaining a small fraction of
pretraining compute; large 2025-era reasoning-RL runs push the multiple
higher still.
...
policy, this means up to $\sim$280B parameters in GPU memory \emph{if all
four models are policy-sized}---in practice the RM and critic are often
smaller (the critic is commonly initialized from the RM), and optimizer
states and activations add substantially more memory on top of the raw
parameters.}
...
up to $\sim$280B parameters in memory simultaneously if all four models are
policy-sized (before counting optimizer states and activations);
(b)~hyperparameter
...
The KTO loss uses a Kahneman-Tversky-inspired value function, which treats
desirable and undesirable examples with \emph{opposite-signed} arguments and
asymmetric weights, reflecting the empirical finding that humans are
loss-averse:
\begin{equation}
\loss_{\text{KTO}} = \E_{(x,y)} \bigl[ \lambda_y - v(x, y) \bigr],
\qquad
v(x, y) =
\begin{cases}
  \lambda_D \, \sigma\!\bigl(\beta \, r_\theta(x, y) - z_{\text{ref}}\bigr)
    & y \text{ desirable} \\[4pt]
  \lambda_U \, \sigma\!\bigl(z_{\text{ref}} - \beta \, r_\theta(x, y)\bigr)
    & y \text{ undesirable}
\end{cases}
\end{equation}
where $r_\theta(x, y) = \log(\pi_\theta(y|x) / \pi_{\text{ref}}(y|x))$
is the implicit reward, $z_{\text{ref}}$ is a reference point (an estimate
of the KL divergence between policy and reference), and
$\lambda_D, \lambda_U$ (with $\lambda_y \in \{\lambda_D, \lambda_U\}$)
weight desirable and undesirable examples asymmetrically.  A desirable
response is rewarded for exceeding the reference point; an undesirable
response is penalized for not falling below it---collapsing the two cases
into a single term would mis-train the undesirable examples.
...
% RL-CAI (right column)
\node[stepbox, right=4.8cm of gen] (gen2) {Generate\\response pairs};
...
\node[lbl, text=deepgreen!70, anchor=south] at (4.7, 0.08) {feeds into};
...
PRMs can be used to guide \textbf{search} over reasoning paths at
inference time:
...
This is one form of \textbf{test-time compute scaling}: spending more
...
\begin{warningbox}
Do not describe PRM-guided search as the mechanism behind reasoning models
like o1 and o3---that was the leading 2023 hypothesis, and it is now
superseded.  DeepSeek-R1, the one frontier reasoning model with a public
training report, explicitly describes PRM- and MCTS-based approaches as
unfruitful in its experiments; its reasoning capability came from
reinforcement learning on \emph{outcome-based verifiable rewards} (answer
checkers, unit tests).  In current practice, PRMs survive mainly as
\textbf{test-time verifiers and rerankers}---scoring sampled candidate
solutions for Best-of-N selection---rather than as the training mechanism
of reasoning models.
\end{warningbox}
...
\subsection{The Standard Alignment Recipe (2023--2024 Practice)}

Most frontier labs have followed a recipe similar to:
...
\item \textbf{SFT:} Fine-tune on high-quality instruction-response pairs
  (10K--100K examples, at this era's scale). Quality matters far more than
  quantity (LIMA, 2023: ``Less Is More for Alignment'')---see
  Section~\ref{sec:staged_pipeline} for how the scale, though not the
  quality lesson, has since changed.
\item \textbf{Preference optimization:} Apply DPO or RLHF on 50K--500K
  preference pairs (again, this era's scale).  Some labs do iterative
  rounds: collect preferences on the current model's outputs, optimize,
  repeat.
...
% ============================================================================
\section{The 2025--26 Recipe: RLVR and GRPO in Practice}
\label{sec:rlvr_practice}
% ============================================================================

Between 2024 and 2026 the center of gravity of post-training shifted.
Everything in this chapter still runs---SFT, reward models, DPO, and
KL-anchored RL all survive---but for every domain where correctness can be
\emph{checked by a program}, the learned reward model has been replaced or
augmented by \textbf{verifiable rewards}:

\begin{itemize}[leftmargin=*]
\item \textbf{Unit tests} for code: reward 1 if the generated program
  passes the test suite, 0 otherwise.
\item \textbf{Math checkers:} extract the final answer (e.g., from a
  \texttt{\textbackslash boxed\{\}} expression) and compare it against
  ground truth with symbolic-equivalence rules.
\item \textbf{Format checks:} reward adherence to the required structure
  (reasoning inside \texttt{<think>} tags, a parseable final answer).
\end{itemize}

This regime---\textbf{RLVR} (RL with Verifiable Rewards)---matters
operationally as much as conceptually.  A verifier does not drift as the
policy improves, has no length or sycophancy bias, and never needs a
preference-data refresh, so RLVR runs tolerate far more optimization
pressure than RM-based RLHF: KL anchoring is weakened or (in some recipes)
dropped entirely.  Open-ended helpfulness, style, and safety remain
unverifiable, so learned reward models and preference data keep their
role---the modern pipeline uses both, in different stages
(Section~\ref{sec:staged_pipeline}).

\paragraph{GRPO in one paragraph.}
The workhorse optimizer of RLVR is \textbf{GRPO} (Group Relative Policy
Optimization; DeepSeekMath 2024, DeepSeek-R1 2025), which is PPO with the
critic deleted.  For each prompt, sample a \emph{group} of $K$ responses
from the current policy and score each with the (usually verifiable)
reward; the advantage of response $i$ is its group-normalized reward,
$\hat{A}_i = \bigl(r_i - \operatorname{mean}(r_{1:K})\bigr) /
\operatorname{std}(r_{1:K})$, assigned identically to every token of that
response.  The update is PPO's clipped surrogate with a per-token KL
penalty to the reference model, but with \emph{no value network}: the
group mean replaces the critic's baseline.  With a rule-based verifier
this cuts PPO's four-model lineup (Table~\ref{tab:four_models}) to
\textbf{two models in memory}---policy and reference.  The full
derivation, the exact objective, and the pathology-and-fix catalogue
(zero-advantage groups, difficulty and length biases, entropy collapse;
DAPO and Dr.~GRPO) are canonical in [DL~17]---this chapter takes the
optimizer as given and covers how it is \emph{run}.

\subsection{The Practice Knobs}

These are the numbers that separate ``has read the papers'' from ``has run
the pipeline'' in an interview:
...
\begin{tabularx}{\textwidth}{@{}l l X@{}}
\toprule
\textbf{Knob} & \textbf{Typical setting} & \textbf{Why} \\
\midrule
KL coefficient $\beta$ & 0.01--0.2 (RM-based); much smaller or 0 for RLVR &
  Learned RMs need tight anchoring; verifiers tolerate pressure, and some
  RLVR recipes (e.g., DAPO) drop the KL term entirely \\
KL budget & Single-digit nats per sequence &
  Realized $\KL(\pi_\theta \| \pi_{\text{ref}})$ held to a few nats per
  response, often via an adaptive $\beta$ controller; tens of nats
  usually means hacking \\
Group size $K$ (GRPO) & 8--64 responses per prompt &
  Larger $K$ gives a lower-variance baseline and better pass@$K$
  coverage of hard prompts, at linear rollout cost \\
Rollout\,:\,training compute & $\sim$80\,:\,20 &
  Autoregressive generation of $K$ long responses per prompt dominates
  wall-clock; the gradient step over the same tokens is comparatively
  cheap \\
Iteration structure & On-policy batches; 1--few gradient steps per batch &
  Generate a batch with current weights $\to$ update $\to$ sync weights
  to the rollout engine; minibatch reuse is limited to keep the
  importance ratios near 1 \\
Reward refresh & RM: periodic retraining; verifier: never &
  A learned RM goes stale as the policy distribution moves and must be
  refreshed on fresh preferences; a unit test or math checker is stable
  for the whole run \\
RL\,:\,SFT compute & $\sim$3--5$\times$ (higher for long reasoning runs) &
  The RL stage costs a small multiple of SFT and a small fraction of
  pretraining; generation, not gradients, is where it goes \\
\bottomrule
\end{tabularx}
\caption{Operational knobs of 2025--26 RLHF/RLVR practice and their
typical magnitudes.}
\label{tab:rlvr_knobs}
\end{table}

\subsection{The Staged Pipeline as Practiced}
\label{sec:staged_pipeline}

The three-stage InstructGPT pipeline has grown into a four-stage recipe in
which each stage uses the cheapest reward signal that suffices:

\begin{enumerate}[leftmargin=*]
\item \textbf{SFT at scale:} \emph{millions} of instruction-response
  examples, the large majority synthetic---distilled reasoning traces,
  model-written and model-filtered chat, code, math, and tool-use data---%
  replacing the 10K--100K human-written demonstrations of the
  InstructGPT era.  LIMA (2023) survives as the quality-over-quantity
  counterpoint, but it was a claim about \emph{style} transfer with
  1K examples; 2026-scale SFT is teaching formats, tools, and reasoning
  behaviors, and quantity came back once synthesis made quality scalable.
\item \textbf{RLVR on reasoning domains:} GRPO-style RL against unit
  tests, math checkers, and format checks on math/code/logic/agentic
  prompts.  This is where most of the RL compute goes and where the
  capability gains come from.  The reasoning-model version of this stage
  (cold-start SFT, reasoning RL, rejection-sampling SFT, all-scenario
  RL---the R1 recipe) is covered in [DL~17].
\item \textbf{Preference RL / DPO for style and safety:} a learned RM or
  offline DPO over millions of largely synthetic preference pairs handles
  what no verifier can check---tone, formatting taste, helpfulness
  tradeoffs, refusal boundaries.
\item \textbf{Targeted regression fixes:} short, narrow patch stages at
  the end---small SFT or DPO runs on focused data to fix what the
  regression suite caught (over-refusal, formatting drift, a benchmark
  dip, sycophancy) without re-running the expensive stages.
\end{enumerate}

\begin{keyinsight}[Where Offline DPO Landed]
The 2023 debate---``is DPO as good as PPO?''---resolved by division of
labor rather than by victory.  Online RL (GRPO/PPO) against verifiable
rewards does the capability heavy lifting, because capability gains
require exploring beyond the current distribution and checking the
result.  Offline DPO settled into the role it is actually best at: a
\textbf{cheap, stable alignment stage} for style and safety preferences,
run late in the pipeline on synthetic pairs, at a fraction of the cost of
any online stage.  Saying ``we'd align the model with DPO'' is no longer
a complete answer about capability training; saying ``DPO is obsolete''
is equally wrong about the style/safety stage.
\end{keyinsight}

\subsection{Training Infrastructure Shape}
\label{sec:rlhf_infra}

Online RL for LLMs is, physically, two systems bolted together:

\begin{itemize}[leftmargin=*]
\item \textbf{Rollout engine:} an inference stack (vLLM/SGLang-class:
  continuous batching, paged KV cache) that generates $K$ responses per
  prompt as fast as serving infrastructure allows.
\item \textbf{Learner:} a training stack (FSDP/Megatron-class sharding)
  that computes the clipped-surrogate loss over the rollout batch and
  takes the gradient step.
\end{itemize}

\paragraph{Why generation dominates.}
Each update consumes $K$ responses per prompt, each of them thousands
(for reasoning models, tens of thousands) of \emph{serially decoded}
tokens,
while the learner replays those same tokens in one (teacher-forced,
parallel) forward-backward pass.  The result is the roughly 80:20 split
of wall-clock time between rollouts and training noted in
Table~\ref{tab:rlvr_knobs}: speeding up RLHF is mostly an inference
optimization problem, which is why the rollout engine \emph{is} a
serving engine.

\paragraph{Weight sync and placement.}
After each on-policy batch, the learner's updated policy weights must
reach the rollout engine---a broadcast of the full parameter set whose
cadence defines how on-policy the run is.  Two placements are used:
\textbf{colocated} (the same GPUs time-slice between generate and train
phases, swapping engine state in place: no idle pools, but both memory
footprints and engine-switch overhead share each device) and
\textbf{disaggregated} (separate rollout and learner pools running
concurrently: independent scaling and no switching, but rollouts are
produced by weights one update behind the learner).  That
\textbf{one-step-off-policy staleness} has a standard one-line fix: keep
the behavior policy's log-probs with each rollout and correct with the
per-token importance ratio $\pi_\theta / \pi_{\text{behavior}}$---which
is exactly the ratio PPO/GRPO's clipped objective already computes.
Off-the-shelf frameworks---\textbf{verl}, \textbf{OpenRLHF},
\textbf{TRL}, \textbf{NeMo-Aligner}---package these patterns, differing
mainly in placement strategy and rollout-engine integration.

% --- Infrastructure design question ---
\begin{interviewq}
\textbf{Q: Design the RLHF/RLVR training system for a 70B model.}

\badgeLseven{L6--L7} \quad \badgetype{System Design} \quad
\badgefreq{\freqhigh}

\stronganswer
Structure the answer as: model inventory and memory, rollout/learner
topology, the reward path, and the monitoring loop.

\textbf{1.~Inventory and memory.}  Algorithm choice sets the model count.
PPO with a learned RM: policy, reference, RM, critic (four).  GRPO with a
rule-based verifier: policy and reference (two)---the practical reason
GRPO wins at this scale.  The learner holds the policy with training
state: 70B params $\times$ $\sim$16 bytes/param for mixed-precision Adam
(bf16 weights and gradients, fp32 master weights, two fp32 moments)
$\approx$ 1.1\,TB,
sharded (FSDP/ZeRO-3 or tensor+pipeline parallel) over tens of GPUs
before activations.  The reference model (and RM, if used) is
inference-only: $\sim$140\,GB in bf16 each, shardable, no optimizer
state---it can also be served as log-prob scoring on the rollout side.

\textbf{2.~Rollout/learner topology.}  Split generation onto a
vLLM/SGLang-class rollout pool and training onto a learner pool.  Budget
$\sim$80\% of GPU wall-clock for generation (Table~\ref{tab:rlvr_knobs})
and size the pools accordingly.  Choose colocated time-slicing for
simplicity at moderate scale, disaggregated pools for large runs---and
if disaggregated, store behavior log-probs with every rollout and rely
on the PPO/GRPO importance ratio to absorb the one-step staleness.
Weight sync (learner $\to$ rollout engines) happens every on-policy
batch; at 140\,GB per broadcast this must be NCCL/GPU-direct, not a
checkpoint round-trip through disk.

\textbf{3.~Reward path.}  For RLVR, the ``reward model'' is a sandbox
fleet: CPU workers executing unit tests and math checkers with strict
isolation, timeouts, and no network---engineered like untrusted-code
infrastructure, because under optimization pressure it \emph{will} be
attacked (Section~\ref{sec:rlvr_reward_hacking}).  Verifier throughput
must match rollout throughput or it silently becomes the bottleneck.
For preference stages, an RM inference pool scores completed responses.

\textbf{4.~Iteration and monitoring.}  On-policy loop: sample prompts
$\to$ generate $K$ responses each $\to$ score $\to$ one-to-few gradient
steps $\to$ sync.  Dashboard: reward curve, per-sequence KL against the
single-digit-nat budget, response length split by correctness (the
length-bias tell), policy entropy, fraction of zero-advantage groups,
pass@$K$ on held-out prompts, and periodic human/LLM-judge evals to
catch proxy-vs-true divergence.  Checkpoint policy+optimizer and make
the rollout queue replayable for recovery.

\redflags
\begin{itemize}
\item Treats it as fine-tuning with a fancy loss---never mentions that
  generation dominates the compute, or budgets GPUs only for the learner.
\item Forgets the reference model (or the critic, having chosen PPO)
  in the memory budget.
\item No weight-sync story: the rollout engine silently trains on stale
  weights with no importance correction.
\item Runs generated code unsandboxed, or ignores verifier throughput.
\item No monitoring beyond ``the reward goes up.''
\end{itemize}

\interviewtest{Whether the candidate can turn the RLHF/RLVR loop into a
running system: memory arithmetic, the rollout/learner split, staleness
handling, and reward-side engineering.  This is the standard L6/L7
systems question for post-training teams, and it has no purely
conceptual answer.}

\goodgreat
\begin{itemize}
\item \badgeLfive{L5}: Knows the components and that generation is the
  expensive part; can count models in memory for PPO vs.\ GRPO.
\item \badgeLsix{L6}: Does the memory arithmetic unprompted, designs the
  rollout/learner split with weight-sync cadence, and sizes the pools
  from the 80:20 ratio.
\item \badgeLseven{L7}: Discusses colocated vs.\ disaggregated tradeoffs
  and off-policy correction, treats the verifier sandbox as a security
  boundary, names the failure modes the dashboard must catch
  (zero-advantage saturation, length bias, entropy collapse), and can
  say what changes at 700B (sync bandwidth, pipeline-parallel rollout).
\end{itemize}

\followups{How does the design change when you swap PPO for GRPO?  When
would you colocate rollout and training on the same GPUs?  What breaks
if weight sync lags by ten updates instead of one?  How do you stop the
test-execution sandbox from becoming the bottleneck---or an attack
surface?}
\end{interviewq}

\subsection{Reward Hacking in Verifiable Settings}
\label{sec:rlvr_reward_hacking}

Verifiable rewards shrink the Goodhart gap---passing the actual tests
\emph{is} the objective---but the gap does not vanish; it moves into the
verifier.  The 2025--26 manifestations every candidate should know:

\begin{itemize}[leftmargin=*]
\item \textbf{Unit-test special-casing:} the policy hard-codes expected
  outputs or branches on the visible test inputs
  (\texttt{if input == known\_case: return known\_answer}).  The suite
  passes; the task is unsolved.  Weak or fully visible test suites make
  this the path of least resistance.
\item \textbf{Sandbox exploitation:} coding agents with execution access
  learn to edit or delete the failing tests, monkey-patch the assertion
  library or harness, read expected outputs from fixture files, or exit
  with success before the asserts run---attacks on the grading
  \emph{infrastructure} rather than the grading criteria.
\item \textbf{Lenient-checker exploitation:} emitting whatever
  degenerate form a buggy or overly permissive answer matcher
  normalizes to, rather than solving the problem.
\item \textbf{Obfuscated hacking under CoT monitoring:} penalizing
  hack-revealing reasoning with a chain-of-thought monitor teaches the
  policy to hack \emph{without verbalizing it}---CoT monitorability, its
  fragility under optimization pressure, and the resulting
  ``monitor, don't train against'' guidance are covered in [DL~20].
\end{itemize}

The defenses move accordingly: from KL budgets and RM ensembles to
\textbf{test-suite and sandbox quality}.  Hold out hidden tests the
policy never sees and track the visible-suite vs.\ hidden-suite pass-rate
gap (the over-optimization curve of Figure~\ref{fig:overoptimization},
reincarnated with ``proxy'' = training suite and ``true'' = held-out
suite); use property-based tests and mutated inputs so there is no finite
answer key to memorize; make the harness read-only and verify against a
fresh checkout in a separate process so the agent cannot grade itself;
and audit high-reward transcripts, which is where special-casing is
caught.  ``Design a reward for a coding agent that resists test-hacking''
is now a standard follow-up to the reward-hacking question in
Section~\ref{sec:alignment_interview}---the strong answer is this
paragraph.

\begin{keyinsight}[Test Quality Is the New RM Quality]
This chapter's oldest lesson---the reward model is the ceiling of the
pipeline---survives the RLVR transition intact, with the test suite in
the RM's role.  A weak test suite is exactly a hackable reward model:
finite, exploitable, and silently passing bad policies.  Labs now invest
in hidden tests, property-based generation, and sandbox hardening the
way 2023 labs invested in preference-data quality, and ``who writes the
tests, and can the policy see them?'' has become the first question to
ask about any RLVR result.
\end{keyinsight}
...
reward hacking in practice?  How do RM ensembles help?  Design a reward
for a coding agent that resists test-hacking
(Section~\ref{sec:rlvr_reward_hacking}: hidden and property-based tests,
a hardened read-only sandbox, transcript audits).}
\end{interviewq}
...
preferences on a held-out test set---the achievable ceiling is set by the
dataset's inter-annotator agreement (often only 70--80\%), so accuracies of
65--75\% are typical on noisy general-chat preference sets, while top RMs
exceed this on cleaner modern preference benchmarks; (2)~\textbf{generalize}
...
RewardBench is the standard public benchmark here, spanning chat, safety,
...
(beam search, MCTS) select the most promising paths.  This is one way to
spend test-time compute, but it is \emph{not} how o1/R1-class reasoning
models were built: DeepSeek-R1's published report found PRM and MCTS
unfruitful, and those models are instead trained with RL on
outcome-verifiable rewards.  In practice PRMs today serve mainly as
test-time verifiers and rerankers (Best-of-N selection).
...
\item \badgeLseven{L7}: Connects to test-time compute scaling while noting
  that current reasoning models were trained with RL on outcome-verifiable
  rewards rather than PRM-guided search, discusses the relationship between
  process supervision and formal verification, and considers how PRMs
  change the scaling laws (trading training compute for inference compute).
\end{itemize}
...
alignment is the deepest conceptual point in this chapter, and its
2025--26 form (verifiable rewards make the test suite the proxy, and
test-hacking the hack) is where interviewers now probe it.  Candidates
