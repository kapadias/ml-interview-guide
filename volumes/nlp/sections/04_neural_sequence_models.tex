...
\end{equation}
where $\gamma \leq 1$ upper-bounds the derivative of $\tanh$ and
$\lVert \cdot \rVert$ denotes the spectral norm.  If the \textbf{largest
singular value} of $\mW_h$ satisfies $\lVert \mW_h \rVert < 1/\gamma$
(Pascanu et al., 2013), the product decays \emph{exponentially} in
$(t - k)$.  For long-range dependencies (large $t - k$), the gradient
effectively vanishes to zero.
\end{mathresult}
...
% --- Forget gate multiply / input gate add on the cell-state line ---
\node[op] (fmul) at (-3.4, 3.0) {$\times$};
\node[op] (iadd) at ( 0.0, 3.0) {$+$};

% --- Cell state line (the "conveyor belt") ---
\draw[cellarr] (-6.3, 3.0) -- node[above, lbl, pos=0.3]{$\vc_{t-1}$} (fmul);
\draw[cellarr] (fmul) -- (iadd);
\draw[cellarr] (iadd) -- (6.3, 3.0) node[above, lbl, pos=0.8]{$\vc_t$};

% --- Gates (evenly spaced so the labels above them never collide) ---
\node[gate] (fg)   at (-3.4, 1.2) {$\sigma$};
\node[gate] (ig)   at (-1.2, 1.2) {$\sigma$};
\node[gate] (cand) at ( 1.0, 1.2) {$\tanh$};
\node[gate] (og)   at ( 2.8, 1.2) {$\sigma$};
\node[lbl, above=0.12cm of fg]   {forget $\mathbf{f}_t$};
\node[lbl, above=0.12cm of ig]   {input $\mathbf{i}_t$};
\node[lbl, above=0.12cm of cand] {$\tilde{\vc}_t$};
\node[lbl, above=0.12cm of og]   {output $\mathbf{o}_t$};

% --- Inputs and concatenation ---
\node[op] (concat) at (-5.2, 0.15) {\tiny$[\;]$};
\node[lbl] at (-6.5,  0.05) {$\vh_{t-1}$};
\node[lbl] at (-6.5, -0.85) {$\vx_t$};
\draw[arr] (-6.5,  0.28) -- (-6.5,  0.55) -- (-5.2,  0.55) -- (concat.north);
\draw[arr] (-6.5, -0.62) -- (-6.5, -0.25) -- (-5.2, -0.25) -- (concat.south);

% --- Concatenation feeds every gate ---
\foreach \gx/\g in {-3.4/fg, -1.2/ig, 1.0/cand, 2.8/og}{
    \draw[arr] (concat) -- (\gx, 0.15) -- (\g);
}

% --- Forget gate scales the cell state ---
\draw[arr] (fg) -- (fmul);

% --- Input gate times candidate, added to the cell state ---
\node[op] (imul) at (0.0, 2.2) {$\times$};
\draw[arr] (ig)   -- (-1.2, 2.2) -- (imul);
\draw[arr] (cand) -- ( 1.0, 2.2) -- (imul);
\draw[arr] (imul) -- (iadd);

% --- Output branch: tanh of the new cell state, gated ---
\node[gate] (tanh_c) at (4.3, 2.0) {$\tanh$};
\node[op]   (omul)   at (4.3, 0.8) {$\times$};
\draw[arr] (4.3, 3.0) -- (tanh_c.north);
\draw[arr] (tanh_c) -- (omul);
\draw[arr] (og.east) -- (3.8, 1.2) |- (omul.west);

% --- Hidden state outputs ---
\draw[arr] (omul) -- (4.3, -0.4) node[below, lbl]{$\vh_t$};
\draw[arr] (4.3, 0.1) -- (6.0, 0.1) node[right, lbl]{$\vh_t \to$};
...
norm must be carefully controlled.
\end{mathresult}
...
\item \textbf{Element-wise gating}: The forget gate applies element-wise
  ...
  avoids the spectral norm issues of the vanilla RNN.
...
= 1{,}665{,}024 \approx 1.67\text{M parameters per layer}$.
\end{example}
...
\textbf{Convention note:} We write $\mathbf{z}_t$ as gating the
\emph{candidate}.  Cho et al.\ (2014) and PyTorch's \texttt{torch.nn.GRU}
use the opposite convention, with $\mathbf{z}_t$ multiplying the
\emph{previous} state $\vh_{t-1}$ and $(1 - \mathbf{z}_t)$ the candidate.
The two are equivalent up to relabeling
$\mathbf{z}_t \leftrightarrow 1 - \mathbf{z}_t$; just be consistent about
which convention you are using when writing the equations in an interview.
...
% --- Input arrows (the backward feed routes left of the forward cell) ---
\foreach \i in {1,2,3,4} {
  \draw[arr, deepblue!60] (x\i) -- (f\i);
  \draw[arr, deepblue!60] ([xshift=-2mm]x\i.north) -- ++(0,2mm) -- ++(-5mm,0)
        -- ++(0,19mm) -- ++(5mm,0) -- ([xshift=-2mm]b\i.south);
}
...
% --- Concatenation arrows (forward feed routes right of the backward cell) ---
\foreach \i in {1,2,3,4} {
  \draw[arr, deepgreen!70] ([xshift=2mm]f\i.north) -- ++(0,2mm) -- ++(5mm,0)
        -- ++(0,19mm) -- ++(-5mm,0) -- ([xshift=2mm]o\i.south);
  \draw[arr, deepgreen!70] ([xshift=-2mm]b\i.north) -- ([xshift=-2mm]o\i.south);
}
...
RNNs to transformers.  It was introduced by Bahdanau et al.\ (2014,
published at ICLR 2015) to
...
\begin{equation}
\text{score}(\vh_{t-1}^d,\, \vh_i^e)
= \mathbf{v}^T \tanh\!\bigl(\mW_1 \,\vh_{t-1}^d + \mW_2 \,\vh_i^e\bigr)
\label{eq:bahdanau}
\end{equation}
...
Note the decoder state being scored: Bahdanau attention conditions on the
\emph{previous} decoder hidden state $\vh_{t-1}^d$ (written $s_{t-1}$ in
the original paper)---the context vector $\vc_t$ is computed first and
then consumed, together with $\vh_{t-1}^d$ and the previous output token,
to produce the new decoder state $\vh_t^d$.  Luong attention (below)
instead scores with the \emph{current} top-layer decoder state
$\vh_t^d$.  Interviewers occasionally probe exactly this distinction.
...
Luong et al.\ (2015) replaced the additive MLP with \emph{multiplicative}
scores computed from the \emph{current} top-layer decoder state: a bilinear
form $\text{score}(\vh_t^d, \vh_i^e) = (\vh_t^d)^T \mW \vh_i^e$
(``general''), a plain dot product $(\vh_t^d)^T \vh_i^e$, and a concat
variant that mirrors Bahdanau's functional form.  The practical contrast is
simple: a multiplicative score is one highly optimized matrix
multiplication rather than a per-pair MLP, so it is faster and more
parallelizable on modern accelerators, and---once the $1/\sqrt{d_k}$
scaling was added---it gives up nothing in quality.  This is the direct
ancestor of the transformer's scaled dot-product attention
(Chapter~\ref{chap:transformers}): additive attention proved the concept,
multiplicative attention made it hardware-friendly, and scaling made it
robust.  The fuller taxonomy---global vs.\ local attention, parameter
counts, and where additive attention can still win---is covered in the Deep
Learning volume [DL~5], which is the canonical home for that material.
...
\paragraph{The wall-clock numbers that ended the RNN era.}
The decisive argument was not modeling power but \emph{training
throughput}.  A recurrent layer must execute $T$ sequential steps per
sequence: each step is a small kernel that cannot launch until the previous
one finishes, so within a sequence the accelerator sits mostly idle no
matter how wide the hardware is.  A transformer layer trained with teacher
forcing computes all $T$ positions in \emph{one parallel pass}---a handful
of large matrix multiplications, exactly the shape GPUs and TPUs are built
for.  The production anchor: GNMT (Wu et al., 2016), the 8-layer LSTM
system behind Google Translate, trained for roughly six days on 96 K80
GPUs per language pair.  One year later, the Transformer (Vaswani et al.,
2017) exceeded GNMT's BLEU after 3.5 days on \emph{eight} P100s (big
model), and the base model beat GNMT's En$\to$De score after about twelve
hours on those same eight GPUs; the paper's own accounting puts the big
model's En$\to$Fr training cost at $2.3 \times 10^{19}$ FLOPs versus
$1.4 \times 10^{20}$ for GNMT.  An order-of-magnitude drop in GPU-days at
better quality is what actually killed large-scale RNN training; the
$O(1)$ path length and the scaling-law story explain why the gap kept
widening from there.
...
Before transformers, convolutional networks offered a fully
\textbf{parallelizable} alternative to RNNs for text.  The reference model
is \textbf{TextCNN} (Kim, 2014): embed the tokens into
$\mX \in \R^{T \times d}$, apply 1D convolutions of several widths---each
filter of width $h$ slides over $h$ consecutive embeddings and acts as a
learned \textbf{n-gram detector}---then apply \textbf{max-over-time
pooling}, keeping only each filter's single strongest activation
$\hat{c} = \max_i c_i$, and classify the concatenated features.  The
max-pool is the load-bearing insight: it asks ``did this pattern occur
\emph{anywhere} in the text?'', producing a fixed-size, position-invariant
feature vector.  That is exactly what makes the model strong for
classification and unsuitable for generation or long-range structure:
position and order are discarded, and the receptive field is bounded by
the filter width.  Kim's configuration---filter widths $\{3,4,5\}$ with
100 feature maps each and dropout 0.5---remained a strong, cheap
text-classification baseline into the late 2010s; 2026 interviews rarely
probe deeper than the max-pooling insight above.
...
Linear attention & Attention at $O(n)$ cost & Quality gap to softmax attention & 2020 \\
S4 (SSM) & Long range at $O(n)$, constant state & Static, input-independent dynamics & 2022 \\
Mamba & Input-dependent gating at $O(n)$ & Imprecise exact recall and copying & 2023 \\
Hybrid stacks & Recall + cheap long context & Current production frontier & 2024 \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{The Modern Recurrence Revival}
\label{sec:recurrence_revival}

The arc above did not end in 2017.  By 2023, the transformer's own
remaining limitation---$O(n^2)$ attention and a KV-cache that grows without
bound as context grows---had revived this chapter's central idea: a
\emph{fixed-size state carried forward by a gated recurrence}.

\textbf{Selective state spaces are input-dependent gating.}
State space models entered deep learning as linear time-invariant
recurrences ($\vh_t = \mA \vh_{t-1} + \mB \vx_t$; S4, Gu et al., 2022)
that, being linear, can be trained in parallel via an equivalent
convolution while running as an $O(1)$-per-token recurrence at inference.
Their weakness was precisely the time-invariance: constant $\mA$ and
$\mB$ process every token identically, so the model cannot decide what to
store and what to ignore.  \textbf{Mamba} (Gu \& Dao, 2023) makes the
transition quantities \emph{functions of the current input}---``selective''
state spaces.  In particular, the input-dependent step size
$\Delta_t$ interpolates each state dimension between ``keep'' (small
$\Delta_t$) and ``overwrite with the current input'' (large $\Delta_t$).
Compare Eq.~\eqref{eq:lstm_cell}:
$\vc_t = \mathbf{f}_t \odot \vc_{t-1} + \mathbf{i}_t \odot \tilde{\vc}_t$.
This is the forget/input gate pair rediscovered inside the SSM
framework---with the same diagonal, additive gradient path that
Section~\ref{sec:lstm} derived, now applied at transformer scale.  The
price of selectivity is losing the parallel convolutional form; Mamba
recovers training parallelism with a hardware-aware parallel scan.
Mamba-2's \emph{state-space duality} (Dao \& Gu, 2024) closes the circle
in one line: selective SSMs and linear attention are two views of the same
structured-matrix computation, so ``linear attention as an RNN'' and
``SSM'' describe one model family.  Two siblings deserve a line each:
\textbf{RWKV} reformulates attention as an exponentially decaying
recurrence, giving a model that trains in parallel like a transformer but
deploys as an RNN; \textbf{xLSTM} (Beck et al., 2024) upgrades the LSTM
itself with exponential gating and a parallelizable matrix-memory cell,
showing that the 1997 formulation---not gated recurrence as an idea---was
the limitation.

\textbf{Why hybrids won in production.}
A recurrent stack compresses unbounded history into a fixed-size
state---a lossy summary---so pure SSMs falter exactly where attention
shines: exact retrieval of a needle from earlier context, verbatim
copying, and in-context lookups.  Production systems therefore converged
on \emph{hybrids} that interleave a few attention layers among
SSM/linear-recurrence layers, at attention:SSM ratios of roughly 1:5 to
1:8 (Jamba interleaves 1:7; MiniMax-01 places one softmax-attention layer
per eight; IBM Granite 4.0 runs a still leaner $\sim$1:9 stack).  The
sparse attention layers restore exact retrieval; the recurrent layers keep
per-token compute and state constant, shrinking KV-cache memory several-fold
and making long-context serving cheap.  The 2026 answer to ``when would you
still choose recurrence?'' (Q7 below) is therefore no longer an LSTM: it is
an SSM or hybrid stack for streaming, on-device, and long-context serving,
with the LSTM as the historical answer.

The full treatment---the SSM discretization math, Mamba-2/SSD, and the
hybrid design space with its trade-offs---is canonical in the Deep
Learning volume's efficient-architectures chapter [DL~14].  Take this
chapter's LSTM derivations with you when you read it: Mamba's selection
mechanism is gated recurrence reborn, and an interviewer who follows ``why
did transformers replace LSTMs?'' with ``so why is recurrence back?'' is
really asking whether you understood Section~\ref{sec:lstm}'s math well
enough to recognize it in a new costume.
...
where $\tanh'(z) \in (0, 1]$.  If the largest singular value of $\mW_h$
is less than $1/\gamma$ (with $\gamma \leq 1$ bounding the derivative),
this product decays \emph{exponentially} as $(t - k)$ grows.  After even 10--20
...
\item Not mentioning the spectral norm bound or the mathematical mechanism.
\end{itemize}
...
\item \badgeLfive{L5}: Correctly derives the Jacobian product, explains
  why the spectral norm of $\mW_h$ matters, and shows how the LSTM cell
  state creates an additive gradient path.
...
$\mathbf{v}^T \tanh(\mW_1 \vh_{t-1}^d + \mW_2 \vh_i^e)$---
a small MLP with a $\tanh$ nonlinearity, conditioned on the
\emph{previous} decoder state $\vh_{t-1}^d$ ($s_{t-1}$ in the paper).
...
Luong attention uses the \emph{current} top-layer decoder state:
