...
\textbf{Self-consistency sampling}, formalized as SelfCheckGPT (Manakul et al., 2023), is the simplest approach: sample multiple responses at temperature $>0$, extract factual claims, and flag claims that are inconsistent across samples. If the model says ``founded in 1998'' in 3 of 5 samples but ``founded in 2001'' in the other 2, both dates are suspect. Effective but multiplies inference cost by $N$. (Do not confuse this with \textit{self-consistency} in Wang et al., 2023, which votes over sampled chain-of-thought paths to improve reasoning accuracy, not to detect hallucination.)
...
\begin{enumerate}
    ...
    \item \textbf{Citation generation:} Require the model to produce inline citations for factual claims. At verification time, check that cited sources exist and support the claim. Products like Perplexity and Microsoft Copilot (formerly Bing Chat) use this pattern.
...
\textbf{Note:} RAG system design is covered in detail in Chapter~\ref{chap:long_context_rag}. This section focuses specifically on how RAG reduces hallucination.
