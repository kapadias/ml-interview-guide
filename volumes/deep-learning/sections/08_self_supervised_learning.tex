...
\begin{tikzpicture}[
    ...
    level2/.style={box, fill=lightyellow, minimum width=1.7cm}
]
    % Root
    \node[rootbox] (root) {Self-Supervised Learning};
    
    % Level 1
    \node[level1, below left=1.2cm and 1.2cm of root] (contrastive) {Contrastive};
    \node[level1, below=1.2cm of root] (noncontr) {Non-Contrastive};
    \node[level1, below right=1.2cm and 1.2cm of root] (masked) {Masked Prediction};
    
    % Level 2 - Contrastive
    \node[level2, below=0.8cm of contrastive, xshift=-1.15cm] (simclr) {SimCLR};
    \node[level2, below=0.8cm of contrastive, xshift=1.15cm] (moco) {MoCo};
    \node[level2, below=2.1cm of contrastive] (clip) {CLIP};
    
    % Level 2 - Non-contrastive
    \node[level2, below=0.8cm of noncontr, xshift=-1.15cm] (byol) {BYOL};
    \node[level2, below=0.8cm of noncontr, xshift=1.15cm] (simsiam) {SimSiam};
    \node[level2, below=2.1cm of noncontr] (dino) {DINO};
    
    % Level 2 - Masked
    \node[level2, below=0.8cm of masked, xshift=-1.15cm] (bert) {BERT};
    \node[level2, below=0.8cm of masked, xshift=1.15cm] (mae) {MAE};
    \node[level2, below=2.1cm of masked] (beit) {BEiT};
...
\FOR{each positive pair $(i, j)$}
    \STATE $\ell_{i,j} = -\log \frac{\exp(\text{sim}(z_i, z_j)/\tau)}{\sum_{k=1}^{2N} \mathds{1}_{k \neq i} \exp(\text{sim}(z_i, z_k)/\tau)}$
\ENDFOR
...
\interviewtest{Whether the candidate understands that augmentations are not just regularization but define the semantic content of the learned representation.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Knows the standard augmentation set and that composition matters
    \item \badgeLsix{L6} Explains the invariance principle, can reason about task-specific augmentation design
    \item \badgeLseven{L7} Discusses multi-crop strategies, asymmetric augmentations (DINO), and the information-theoretic view (augmentations control mutual information between views)
\end{itemize}

\followups{``What if your downstream task is texture classification---would you change augmentations?'' ``How do multi-crop strategies in DINO differ from SimCLR's two-crop approach?'' ``Can learned augmentations outperform hand-designed ones?''}
\end{interviewq}
...
\centering
\begin{tikzpicture}[
    box/.style={rectangle, draw, rounded corners, minimum width=1.6cm, minimum height=0.9cm, align=center, font=\footnotesize},
    arrow/.style={->, thick}
]
    % Pipeline
    \node[box, fill=lightyellow] (input) {Image\\patches};
    \node[box, fill=lightyellow, right=0.6cm of input] (visible) {Visible\\patches (25\%)};
    \node[box, fill=lightblue, right=0.6cm of visible] (enc) {Encoder\\(large ViT)};
    \node[box, fill=orange!30, right=0.6cm of enc] (masktok) {Append\\mask tokens};
    \node[box, fill=lightgreen, right=0.6cm of masktok] (dec) {Decoder\\(lightweight)};
    \node[box, fill=lightyellow, right=0.6cm of dec] (recon) {Pixel\\reconstruction};

    % Edges
    \draw[arrow] (input) -- node[above=0.55cm, font=\tiny, align=center] {random\\75\% mask} (visible);
    \draw[arrow] (visible) -- (enc);
    \draw[arrow] (enc) -- (masktok);
    \draw[arrow] (masktok) -- (dec);
    \draw[arrow] (dec) -- node[above=0.55cm, font=\tiny, align=center] {MSE on masked\\patches only} (recon);

    % Annotations
    \node[below=0.3cm of enc, font=\tiny, align=center] {processes only\\visible patches};
    \node[below=0.3cm of masktok, font=\tiny, align=center] {shared learned\\mask token};
    \node[below=0.3cm of dec, font=\tiny, align=center] {few layers, discarded\\after pre-training};
\end{tikzpicture}
\caption{MAE architecture: 75\% of patches are randomly masked, the large encoder processes only the visible 25\%, mask tokens are appended, and a lightweight decoder reconstructs the masked pixels.}
...
data2vec & Any (trained per modality) & 50\% & Latent features & Modality-agnostic objective \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{equation}
I(X; Y) \geq \log(K+1) - \mathcal{L}_{InfoNCE}
\end{equation}
where $K$ is the number of negative samples, so $K+1$ is the total number of candidates scored by the softmax. (CPC writes the bound as $\log N$ with $N$ total candidates; we count negatives throughout this chapter, hence $N = K+1$.)
...
\stronganswer I would design a two-stage pipeline: SSL pre-training followed by supervised fine-tuning.
...
\interviewtest{End-to-end pipeline thinking: method selection, hyperparameter awareness, evaluation strategy, and practical engineering decisions.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Knows the pre-train then fine-tune workflow, picks a reasonable method
    \item \badgeLsix{L6} Justifies method selection based on task and compute, discusses layer-wise LR decay, multi-stage evaluation
    \item \badgeLseven{L7} Considers domain gap between unlabeled and labeled data, discusses curriculum strategies, addresses when SSL might \textit{not} help (e.g., very different domains), mentions semi-supervised alternatives
\end{itemize}

\followups{``What if the unlabeled data is from a different domain?'' ``How would you decide between MAE and DINO?'' ``At what labeled data size does SSL stop being worth the pre-training cost?''}
\end{interviewq}
...
\stronganswer These three methods represent the three major SSL paradigms---contrastive, non-contrastive, and masked prediction---and the choice depends on four factors: compute budget, batch size constraints, downstream task type, and architecture preference.
...
\interviewtest{Ability to make principled architecture decisions based on constraints rather than reciting method descriptions.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Describes each method correctly and names one tradeoff per method
    \item \badgeLsix{L6} Provides a structured decision framework, discusses compute and batch size constraints
    \item \badgeLseven{L7} Mentions linear probe vs.\ fine-tuning gap (MAE is weaker on linear probe), discusses DINOv2 as a unified solution, and considers the data domain
\end{itemize}

\followups{``What if you only have 4 GPUs?'' ``Why does MAE have weaker linear probe performance?'' ``How does DINOv2 combine the best of contrastive and masked approaches?''}
\end{interviewq}
...
\stronganswer I would systematically investigate several hypotheses in order of likelihood.
...
\textit{Hypothesis 3: Training duration.} SSL methods often need hundreds of epochs, so check what was actually held constant across the two runs. At a fixed step budget (same wall-clock compute), 10$\times$ more data means 10$\times$ fewer passes over each example---the model may simply be under-trained relative to the larger dataset. If instead epochs were held constant, the 10M run used 10$\times$ the compute, and the comparison conflates data scale with compute scale. Check: compare runs at matched compute and verify the pre-training loss curves have converged; if the 10M-image loss is still decreasing, the plateau is under-training, not a data limit.
...
\interviewtest{Systematic debugging of ML pipelines, ability to form and prioritize hypotheses.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Mentions data quality and training duration
    \item \badgeLsix{L6} Provides a structured hypothesis-driven approach, discusses capacity scaling
    \item \badgeLseven{L7} Connects to scaling laws for SSL (data vs.\ model size vs.\ compute tradeoffs), discusses how to experimentally distinguish hypotheses efficiently
\end{itemize}

\followups{``How would you decide between scaling the model vs.\ improving data quality?'' ``What metrics would you track during pre-training to catch this early?'' ``How do you detect near-duplicates at scale?''}
\end{interviewq}
...
\stronganswer BERT uses masked language modeling (MLM): mask 15\% of tokens, predict them from bidirectional context. GPT uses causal language modeling (CLM): predict each token from left context only. The tradeoffs are fundamental.
...
\textit{Training signal density.} MLM provides a gradient signal for only 15\% of tokens per forward pass---the masked ones. CLM provides a signal for every token (100\%), so GPT computes $\sim$6.7$\times$ more loss terms per sequence. Be careful not to oversell this as data efficiency: loss coverage is not sample efficiency. At matched scale, MLM-family models were \emph{more} sample-efficient on NLU benchmarks (RoBERTa vs.\ GPT-2), and masking rates well above 15\% can work better (Wettig et al., ``Should You Mask 15\%?''). The defensible claim is more loss terms per FLOP, which compounds with the interface and scaling advantages below.
...
\interviewtest{Understanding of how pre-training objectives shape model capabilities and limitations.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Knows bidirectional vs.\ causal distinction and typical use cases
    \item \badgeLsix{L6} Discusses signal density, train-test mismatch, and scaling behavior
    \item \badgeLseven{L7} Connects to why decoder-only won the scaling race, discusses ELECTRA as an alternative that gives 100\% signal with bidirectional context
\end{itemize}

\followups{``Why not just use bidirectional attention in a decoder?'' ``What about T5's span corruption---does it get the best of both?'' ``How does ELECTRA solve the signal density problem?''}
\end{interviewq}
...
\stronganswer BERT-base has 110M parameters. I'll estimate the total FLOPs and translate to GPU-hours.

\textit{Data size.} 16GB of text at $\sim$4 bytes/token (BPE) $\approx$ 4B tokens. With sequence length 512, training processes 1M steps $\times$ 256 batch size $\times$ 512 seq length $\approx$ 131B total tokens, which is $\sim$33 epochs over the 4B-token corpus.
...
\textit{Sanity check.} The original BERT paper trained on 16 TPUv3 chips for $\sim$4 days. TPUv3 is roughly similar to V100 in FLOPS. 16 $\times$ 96 hours = 1536 GPU-hours, which is higher than my estimate---attributable to older-generation chips with lower peak throughput and a 2018-era training stack achieving well below 50\% MFU. My A100 BF16 estimate is reasonable for modern hardware.
...
\interviewtest{Back-of-envelope estimation skills, understanding of the relationship between model size, data, and compute.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Gets the right order of magnitude with reasonable assumptions
    \item \badgeLsix{L6} Uses the $6P$ rule, accounts for MFU, provides a sanity check
    \item \badgeLseven{L7} Discusses Chinchilla scaling (BERT is massively over-trained relative to its size), estimates memory requirements, and identifies the batch-size/learning-rate tradeoff
\end{itemize}

\followups{``How would the cost change for BERT-large?'' ``Is BERT compute-optimal by Chinchilla standards?'' ``What is the memory footprint during training?''}
\end{interviewq}
...
\stronganswer This is one of the most important questions in modern AI history, and the answer comes down to three interconnected factors: task alignment, scaling properties, and interface universality.
...
\textit{Scaling properties.} Three aspects: (1) Training signal density---CLM computes a loss term for 100\% of tokens vs.\ MLM's 15\%, roughly $6.7\times$ more loss terms per sequence; note this is loss coverage rather than settled sample efficiency (MLM models were more sample-efficient on NLU at matched scale, and higher masking rates or UL2-style mixtures narrow the gap). (2) The loss function provides a natural scaling metric (perplexity) that correlates smoothly with model capability. (3) Emergent abilities---in-context learning, chain-of-thought reasoning---appear in large autoregressive models but not in encoders, because these capabilities require the sequential generation interface.
...
\interviewtest{Deep understanding of why architectural choices matter and how they interact with scale.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Explains the generation vs.\ understanding distinction
    \item \badgeLsix{L6} Discusses signal density, interface universality, and the scale at which decoders catch up on NLU
    \item \badgeLseven{L7} Connects to the ``bitter lesson'' (scale + simple objective beats clever architecture), discusses why T5 (encoder-decoder with span corruption) was an intermediate step, and mentions recent work on non-causal generative models
\end{itemize}

\followups{``Could you design a bidirectional model that generates well?'' ``Why didn't T5's encoder-decoder approach win the scaling race?'' ``What about diffusion-based language models?''}
\end{interviewq}
...
\stronganswer This is one of the most surprising results in SSL. Without negatives pushing representations apart, the loss landscape has a trivial global minimum: map everything to the same constant vector (zero loss). Yet BYOL avoids this through three complementary mechanisms.
...
\textit{Implicit regularization from batch statistics.} BatchNorm (used in the projector/predictor) provides implicit variance regularization across the batch, and early analyses hypothesized it acted as an implicit contrastive term. However, the BYOL authors' follow-up showed this is \emph{not} load-bearing: BYOL trains successfully with GroupNorm + weight standardization, which uses no batch statistics at all. BatchNorm mainly aids optimization/initialization robustness; the predictor + stop-gradient/EMA dynamics are the essential collapse-prevention mechanism.
...
\interviewtest{Depth of understanding of collapse prevention---can the candidate go beyond method descriptions to explain the mechanism?}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Names the three mechanisms (asymmetry, momentum, batch stats)
    \item \badgeLsix{L6} Explains \textit{why} each mechanism prevents collapse, mentions SimSiam as the simplified case
    \item \badgeLseven{L7} Discusses the implicit regularization theory, connects to Barlow Twins/VICReg as explicit decorrelation alternatives, and mentions the ongoing theoretical debate
\end{itemize}

\followups{``What happens if the predictor is too large?'' ``How does VICReg explicitly prevent collapse?'' ``Is BatchNorm necessary for BYOL to work?''}
\end{interviewq}
...
\stronganswer Medical imaging adds several domain-specific challenges to the standard SSL pipeline. I would use a multi-stage approach.
...
\interviewtest{Ability to adapt general SSL knowledge to a specific domain with practical constraints.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Knows the pre-train then fine-tune workflow, picks a reasonable method
    \item \badgeLsix{L6} Adapts augmentations for medical domain, discusses class imbalance, considers interpretability
    \item \badgeLseven{L7} Discusses regulatory constraints (FDA approval requires interpretability), addresses distribution shift across imaging equipment, considers federated SSL for multi-hospital data
\end{itemize}

\followups{``Would you use ImageNet pre-trained weights as initialization for SSL?'' ``How do you handle different imaging modalities (CT, MRI, X-ray)?'' ``How would you validate that SSL features capture clinically relevant patterns?''}
\end{interviewq}
...
\stronganswer InfoNCE is the core loss function for contrastive SSL. For a positive pair $(i, j)$ with $K$ negatives:

$$\loss_{\text{InfoNCE}} = -\log \frac{\exp(\text{sim}(z_i, z_j)/\tau)}{\sum_{k=1}^{K+1} \exp(\text{sim}(z_i, z_k)/\tau)}$$

\textit{Connection to mutual information.} Oord et al.\ (CPC, 2018) showed that minimizing InfoNCE maximizes a lower bound on mutual information: with $K$ negatives, i.e.\ $K+1$ candidates in the denominator, $I(X; Y) \geq \log(K+1) - \loss_{\text{InfoNCE}}$. (CPC states the bound as $\log N$ with $N$ total candidates; under this chapter's convention of counting negatives, $N = K+1$.) Intuitively, if the model can reliably identify the positive among $K$ negatives, the representations must capture shared information between views. With more negatives, the bound becomes tighter, explaining why larger batch sizes help contrastive methods.

\textit{Why collapse gives non-minimal loss.} If all representations collapse to a constant, every pair has similarity 1, so the softmax is uniform over the $K+1$ candidates and the loss becomes $\log(K+1)$---the chance-level value, far from the minimum of 0 (positive similarity $\gg$ all negative similarities). Note this is not the maximum: the loss is unbounded above, since negatives scoring higher than the positive push it beyond $\log(K+1)$. Collapse is therefore a mediocre solution rather than the worst one, and the negatives ensure it is never a minimum---this is why contrastive methods naturally resist collapse.
...
\interviewtest{Mathematical understanding of the contrastive objective and its information-theoretic motivation.}

\goodgreat
\begin{itemize}
    \item \badgeLfive{L5} Writes the loss correctly, explains what positive and negative pairs are
    \item \badgeLsix{L6} Explains the MI lower bound, why collapse gives non-minimal loss, and temperature's role
    \item \badgeLseven{L7} Discusses tightness of the MI bound, false negatives problem, and alternatives like SigLIP that use sigmoid instead of softmax
\end{itemize}

\followups{``What are false negatives in contrastive learning and how do they hurt?'' ``How does SigLIP's loss differ from InfoNCE?'' ``Why does CLIP use a learnable temperature?''}
\end{interviewq}
