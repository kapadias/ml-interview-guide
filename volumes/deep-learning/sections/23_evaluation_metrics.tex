...
\begin{tikzpicture}[
    ...
    decision/.style={diamond, draw, aspect=2, inner sep=1pt, fill=lightyellow, align=center, font=\footnotesize},
    result/.style={rectangle, draw, rounded corners, fill=lightgreen, text width=1.45cm, minimum height=0.8cm, align=center, font=\scriptsize},
    arrow/.style={->, thick, >=stealth}
]
    % Leaves sit on one evenly spaced row; each decision node is centred over its
    % own pair, so no two shapes can reach each other.
    % Root
    \node[decision] (start) at (6.475, 0) {What is your\\task?};

    % Level 1 branches
    \node[decision] (clf)  at (0.925, -2.8) {Classes\\balanced?};
    \node[decision] (rank) at (4.625, -2.8) {Graded\\relevance?};
    \node[decision] (reg)  at (8.325, -2.8) {Outliers\\present?};
    \node[decision] (gen)  at (12.025, -2.8) {Reference\\text available?};

    % Classification results
    \node[result] (rocauc) at (0,    -5.0) {ROC-AUC\\+ F1};
    \node[result] (prauc)  at (1.85, -5.0) {PR-AUC\\+ Recall};

    % Ranking results
    \node[result] (ndcg) at (3.7,  -5.0) {NDCG};
    \node[result] (map)  at (5.55, -5.0) {MAP /\\MRR};

    % Regression results
    \node[result] (rmse) at (7.4,  -5.0) {RMSE\\+ $R^2$};
    \node[result] (mae)  at (9.25, -5.0) {MAE\\/ Huber};

    % Generation results
    \node[result] (auto) at (11.1,  -5.0) {BLEU /\\ROUGE};
    \node[result] (ppl)  at (12.95, -5.0) {Perplexity\\/ Human};
