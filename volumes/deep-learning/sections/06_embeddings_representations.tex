...
\textbf{Caveat}: the canonical result depends partly on the evaluation protocol---the standard analogy test excludes the input words (``king,'' ``man,'' ``woman'') from the candidate set, and without that exclusion the nearest neighbor of the arithmetic result is often ``king'' itself (Nissim et al., 2020). Present the example as evidence of approximate linear regularities in the space, not as exact vector arithmetic; an expert interviewer will push back on the uncritical version.
