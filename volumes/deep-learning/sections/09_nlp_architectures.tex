...
\begin{tikzpicture}[
    node distance=0.8cm,
    box/.style={rectangle, draw, minimum width=2.1cm, minimum height=0.7cm, rounded corners},
    ...
    \begin{scope}[shift={(8,0)}]
        ...
        \node[decoder, right=0.8cm of ed_e1] (ed_d1) {Decoder};
        ...
        \node[below=1.55cm of $(ed_e1)!0.5!(ed_d1)$, font=\bfseries] {Encoder-Decoder};
        \node[below=2.05cm of $(ed_e1)!0.5!(ed_d1)$, font=\small] {(T5, BART)};
...
\textbf{Scale, carefully stated}: the 2022-era finding was that few-shot ICL appeared only above roughly 10B parameters. Treat that as a fact about 2022 recipes, not a law: instruction tuning, curated reasoning data and distillation have since moved competent ICL into models one to two orders of magnitude smaller, and part of the apparent sharpness was a discontinuous metric rather than a discontinuous capability. Chapter~\ref{chap:learning_theory} works the emergence debate properly; quoting a parameter threshold as though it were a constant is a red flag.
...
\begin{tikzpicture}[
    ...
    % No generation
    \node[decision, below=1.5cm of gen, xshift=-3.5cm] (und) {Task type?};
    \node[result, below=1.2cm of und, xshift=-1.5cm] (bert) {BERT/RoBERTa};
    \node[result, below=1.2cm of und, xshift=1.5cm] (deberta) {DeBERTa};
    
    % Generation
    \node[decision, below=1.5cm of gen, xshift=3.5cm] (cond) {Conditional?};
    \node[result, below=1.2cm of cond, xshift=-1.5cm] (t5) {T5/BART};
    \node[result, below=1.2cm of cond, xshift=1.5cm] (gpt) {GPT/LLaMA};
