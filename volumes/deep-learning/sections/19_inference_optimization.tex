...
\centering
\begin{tikzpicture}[
    block/.style={rectangle, draw, text width=1.35cm, align=center, minimum height=0.65cm, font=\footnotesize},
    req/.style={block, fill=lightblue},
    free/.style={block, fill=lightgray, dashed},
    shared/.style={block, fill=lightgreen},
    page/.style={block, fill=lightblue, text width=1.85cm, minimum height=0.75cm},
    arrow/.style={->, thick, >=stealth},
    label/.style={font=\small\bfseries, align=center}
]
    % Logical view: three contiguous pages of one request
    \node[label] at (6.02, 3.7) {Logical View\\(per request)};

    \node[page] (l0) at (3.62, 2.4) {Page 0\\[-2pt] \scriptsize tokens 0--15};
    \node[page] (l1) at (6.02, 2.4) {Page 1\\[-2pt] \scriptsize tokens 16--31};
    \node[page] (l2) at (8.42, 2.4) {Page 2\\[-2pt] \scriptsize tokens 32--47};
    \draw[arrow] (l0) -- (l1);
    \draw[arrow] (l1) -- (l2);

    % Physical view: one row of blocks, this request's three scattered through it
    \node[shared] (p3) at (0,     0) {Block 3};
    \node[free]   (p7) at (1.72,  0) {Block 7};
    \node[req]    (p1) at (3.44,  0) {Block 1};
    \node[req]    (p9) at (5.16,  0) {Block 9};
    \node[free]   (p2) at (6.88,  0) {Block 2};
    \node[req]    (p5) at (8.60,  0) {Block 5};
    \node[free]   (p4) at (10.32, 0) {Block 4};
    \node[shared] (p6) at (12.04, 0) {Block 6};

    \node[font=\footnotesize\itshape] at (1.72,  -0.75) {(free)};
    \node[font=\footnotesize\itshape] at (6.88,  -0.75) {(free)};
    \node[font=\footnotesize\itshape] at (10.32, -0.75) {(free)};

    \node[label] at (6.02, -1.55) {Physical GPU Memory\\(non-contiguous blocks)};

    % Page table: logical page -> physical block
    \draw[arrow, deepblue] (l0.south) -- (p1.north);
    \draw[arrow, deepblue] (l1.south) -- (p5.north);
    \draw[arrow, deepblue] (l2.south) -- (p9.north);
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    % Draft phase
    \node[label, anchor=west] at (-0.8, 2.15) {Draft model (fast):};
    ...
    % Verify phase
    \node[label, anchor=west] at (-0.8, 0.65) {Target model (verify):};
    ...
    % The whole draft row is verified in one target-model pass
    \draw[arrow, dashed, gray] (d3.south) -- (v3.north)
        node[midway, right=0.12cm, font=\footnotesize\itshape, gray] {single forward pass};

    % Result
    \node[label, anchor=west] at (-0.8, -0.75) {Result:};
