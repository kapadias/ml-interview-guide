...
\begin{tldr}
This chapter is the program's \textbf{quantitative-systems} home for attention: parameter counting, FLOPs per token, KV-cache byte math, the MHA $\to$ GQA/MQA $\to$ MLA cache ladder, FlashAttention 1--3 and Flash-Decoding, trainable sparse attention (NSA, MoBA, DSA), and the GPT-2 $\to$ LLaMA $\to$ 2024--26 design evolution with worked LLaMA and H100 anchors. It also carries the attention lineage (Bahdanau/Luong $\to$ scaled dot-product), RoPE at working depth---the base-frequency knob that the context-extension methods in Chapter~\ref{chap:long_context_rag} turn---and the modern design choices (pre-norm, RMSNorm, SwiGLU, QK-norm, interleaved sliding-window attention, large vocabularies). The step-by-step \textit{derivations}---self-attention from first principles, transformer block anatomy, and the positional-encoding schemes other than RoPE---are canon in [NLP 5] and are pointed to rather than repeated. Come here for the arithmetic that decides what fits on a GPU.
\end{tldr}
...
\subsection{Formulation and Parameter Cost}

A single head learns one relationship type; $h$ heads run in parallel over $d/h$-dimensional subspaces so that one layer can attend on several criteria at once, with the output projection $\mW^O$ mixing them. The motivation, the subspace/ensemble argument, and why single-head attention underperforms are developed in [NLP 5]. What the rest of this chapter needs from multi-head attention is its shape and its parameter cost.
...
BERT-era probing catalogued head types---positional, syntactic, rare-word, delimiter, previous-token---and that taxonomy is surveyed in [NLP 5]. The answer that matters at staff level is circuit-level, not taxonomic.
...
\subsection{Block Anatomy: What This Chapter Assumes}

A Transformer layer is two residual sublayers: multi-head self-attention followed by a position-wise FFN, each wrapped in a residual connection and a normalization. The encoder block runs unmasked self-attention over the whole sequence; the decoder block adds a \textbf{causal mask} ($M_{ij} = -\infty$ for $j > i$, added to the logits before the softmax, so position $i$ sees only $j \leq i$) and, in encoder--decoder models, a cross-attention sublayer whose queries come from the decoder stream while keys and values come from the encoder stack. The FFN expands by a factor $f$: $f = 4$ for a two-matrix GELU FFN, $f \approx 8/3$ for SwiGLU, whose gating adds a third matrix at matched parameter count.

The block-by-block walkthrough, the encoder--decoder diagram, and the encoder-only / decoder-only / encoder--decoder family comparison are canon in [NLP 5] and are not repeated here. This chapter treats the block as a \textit{cost model}: every parameter, FLOP, and KV-cache byte counted in Section~\ref{sec:transformer_compute} falls straight out of the structure above---$4d^2$ of attention projections and $2fd^2$ of FFN weights per layer, $2 n_{kv} d_{head}$ cached elements per token per layer.
% label preserved from the encoder-block figure; the diagram is now carried by [NLP 5]
...
Pre-norm trains stably without warmup and scales past 100 layers because the residual path is clean; post-norm can edge it out on final quality when it converges, but needs warmup and is rare in modern LLMs. The full trade-off---including DeepNet-style initialization and Gemma-2's sandwich norm---is worked in the interview question in Section~\ref{sec:attention_interview}, and the normalization mechanics themselves are Chapter~\ref{chap:activation_normalization}.
...
Self-attention is permutation equivariant (Section~\ref{sec:self_attention}), so position must be injected explicitly---otherwise ``The cat sat on the mat'' and ``mat the on sat cat The'' produce identical outputs and the model is a bag of tokens. The schemes differ in \textit{where} position enters: added to the token embedding (sinusoidal, learned absolute), added to the attention score (Shaw-style relative, ALiBi), or applied as a rotation to $\mQ$ and $\mK$ themselves (RoPE).

\begin{table}[H]
\centering
\caption{Positional encoding schemes. $L$ = maximum training length, $K$ = relative-distance clip, $d$ = model dimension, $h$ = number of heads.}
\begin{tabularx}{\textwidth}{llXX}
\toprule
\textbf{Scheme} & \textbf{Extra params} & \textbf{Extrapolation} & \textbf{Where position enters / used in} \\
\midrule
Sinusoidal & none & Degrades empirically & Added to embeddings; original Transformer \\
Learned absolute & $L \times d$ & None---hard ceiling at $L_{\max}$ & Added to embeddings; BERT, GPT-2, early GPT-3 \\
Relative (Shaw) & $O(Kd)$ & To the clip distance & Added to scores (and optionally values); Shaw et al.\ (2018), with related schemes in Transformer-XL and T5 \\
RoPE & none & Good, with base scaling & Rotation applied to Q and K; LLaMA, Mistral, Qwen, DeepSeek \\
ALiBi & $h$ slopes & Strong & Linear distance penalty on scores; BLOOM, MPT \\
\bottomrule
\end{tabularx}
\end{table}

\textbf{Where the derivations live.} The sinusoidal linear-shift property, Shaw-style relative attention, ALiBi's slope construction, and the scheme-by-scheme trade-off discussion are canon in [NLP 5] and are not repeated here. This chapter carries only RoPE at working depth, because the systems material downstream depends on it: the context-extension toolbox in Chapter~\ref{chap:long_context_rag} (position interpolation, NTK-aware scaling, YaRN) manipulates its base frequency directly, MLA's decoupled key (Section~\ref{sec:mla}) exists because of how RoPE composes with cached K/V, and the 2024--26 architecture table turns on the same parameter.
...
f(\vx, m) = \mR_m \vx
\end{equation}
where $\mR_m$ is block-diagonal, rotating each dimension pair $(2i, 2i{+}1)$ by angle $m\theta_i$ with base frequencies
\begin{equation}
\theta_i = b^{-2i/d}, \qquad i = 0, \ldots, d/2 - 1,
\end{equation}
for a base $b$ ($b = 10{,}000$ in the original; $500\text{K--}1\text{M}$ in 2024--26 long-context models).
\end{mathresult}

Written out over the 2D subspaces:
\begin{equation}
\mR_m = \begin{pmatrix}
...
\textbf{Why RoPE is ``relative in disguise''}: the rotations are applied to \textit{absolute} positions $m$ and $n$, one token at a time, but rotation matrices compose, so the score depends only on the offset:
\begin{equation}
(\mR_m \vq)^T (\mR_n \vk) = \vq^T \mR_m^T \mR_n \vk = \vq^T \mR_{n-m} \vk
\end{equation}
There is no relative-position lookup table and no term added to the score matrix---which is why RoPE composes with FlashAttention and with a KV cache unchanged: each key is rotated once, at its own position, \textit{before} it is written to the cache.

\textbf{The base $b$ is the knob.} Dimension pair $i$ rotates with wavelength $2\pi b^{2i/d}$: low-index pairs spin fast (fine, local resolution), high-index pairs spin slowly (coarse, global position). Two consequences drive everything downstream. (1) Beyond the trained length, the model meets rotation angles it never saw---the out-of-distribution failure that makes naive extrapolation break. (2) Every context-extension method is an intervention on this frequency ladder: position interpolation rescales positions uniformly, NTK-aware scaling and YaRN raise $b$ (and rescale non-uniformly) so that the slow, global bands stretch while the fast, local bands are preserved, and modern pretraining simply starts at $b = 500\text{K--}1\text{M}$ so 128K--1M contexts are resolvable by construction. The methods, their extension ratios, and their trade-offs are Chapter~\ref{chap:long_context_rag}.

\textbf{Used in}: LLaMA, Mistral, Qwen, DeepSeek---essentially every modern open-weight LLM.

\textbf{Advantages}:
\begin{itemize}
    \item Relative position for free, through an absolute mechanism
    \item No learned position parameters and no $L_{\max}$ ceiling
    \item Attention decays gently with distance (an implicit recency bias)
    \item Cheap and cache-friendly: two multiply--adds per dimension pair, applied to Q and K only
\end{itemize}
...
Norm placement & Pre-norm (moved from the 2017 Transformer's post-norm) & Pre-norm & Pre-norm + QK-norm \\
...
\begin{itemize}
    ...
    \item \textbf{RoPE base scaling}: the RoPE base grew from 10K to \textbf{500K (Llama 3.1) and 1M+} so that rotation frequencies stay resolvable across 128K--1M contexts, combined with the context-extension toolbox (the base-frequency mechanism is Section~\ref{sec:positional}; position interpolation, NTK-aware scaling, and YaRN are Chapter~\ref{chap:long_context_rag}). Long context became a pretraining design parameter, not a fine-tuning afterthought.
...
\begin{enumerate}
    \item \textbf{LayerNorm $\to$ RMSNorm}: Both are pre-norm. GPT-2 had already moved the
    normalization to each sub-block's input, and that placement is what stabilizes deep
    training---a detail candidates routinely get backwards, so state it carefully. The 2023
    change is the normalizer itself: RMSNorm drops the mean-centering term, runs 10--15\%
    faster, and matches quality.
