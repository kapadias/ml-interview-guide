...
\textbf{Strong answer:} Flash Attention computes \textit{exactly} the same result as standard attention, but reorganizes the computation to minimize memory I/O:

\begin{enumerate}
    \item \textbf{Problem}: Standard attention materializes the full $n \times n$ matrix in slow HBM memory (40 GB on A100, $\sim$2 TB/s bandwidth). The bottleneck is reading/writing this matrix, not computing it---attention is memory-bound, not compute-bound.
    \item \textbf{Solution}: Process attention in tiles that fit in fast SRAM (20 MB on A100, $\sim$19 TB/s bandwidth). Use the online softmax trick to compute exact softmax incrementally without ever materializing the full matrix. Each tile computes a partial softmax, maintaining running max and sum statistics that get corrected as new tiles arrive.
...
\begin{itemize}
    \item \textbf{L5:} Explains the tiling approach and that it's exact, not approximate. Knows memory drops from $O(n^2)$ to $O(n)$.
    \item \textbf{L6:} Can explain the online softmax trick, knows the SRAM/HBM sizes on A100, understands the arithmetic intensity shift from memory-bound to compute-bound. Mentions Flash Attention 2 improvements (better parallelism, causal masking optimization).
    \item \textbf{L7:} Discusses implications for system design: Flash Attention changes which operations are the bottleneck (now FFN, not attention). Knows that FA doesn't help for decode phase (which is memory-bound for a different reason: loading model weights, not attention matrix). Can reason about when IO-aware algorithms matter beyond attention.
\end{itemize}

\textbf{Common follow-ups:} ``What's the online softmax trick?'' ``When would Flash Attention NOT help?'' (Small sequences where attention isn't the bottleneck; decode phase where the bottleneck is weight loading, not attention matrix.) ``What changed in Flash Attention 2 vs 1?'' (Better work partitioning across thread blocks, reduced non-matmul FLOPs.)
\end{interviewq}
...
\textbf{Strong answer:} Break it into four components:
...
\begin{enumerate}
    ...
    \item \textbf{Activations}: For a 32-layer model with $d=4096$, seq=2048, batch=1: each layer stores attention input, QKV projections, attention scores, FFN intermediates. Roughly $\sim$34 \textrm{bytes/param} $\times$ seq $\times$ batch for typical Transformers $\approx$ 16--30 GB (highly dependent on checkpointing)
\end{enumerate}

\textbf{Total}: $\sim$130--140 GB without gradient checkpointing. With mixed-precision training (BF16 compute, FP32 master weights).

\textbf{To fit on 1$\times$ A100 80GB}: Need ZeRO-3 (shard optimizer states + gradients + params across GPUs), or at minimum ZeRO-2 + gradient checkpointing on 2$\times$ A100.
...
\textbf{What the interviewer is testing:} Can you do back-of-envelope calculations for ML systems? Do you know the memory breakdown of training? Can you reason about what fits on which hardware?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Identifies all 4 memory components. Gets the right order of magnitude (>100 GB). Mentions mixed precision as a solution.
    \item \textbf{L6:} Gives precise byte counts per component. Knows Adam has 12 bytes/param overhead (FP32 master + momentum + variance). Can recommend specific ZeRO stages. Knows gradient checkpointing trades $\sim$33\% more compute for $\sim$60\% activation memory savings.
    \item \textbf{L7:} Discusses memory fragmentation overhead ($\sim$10--20\% extra). Knows activation memory depends on sequence length quadratically (attention) vs linearly (FFN). Can compare FSDP vs DeepSpeed ZeRO tradeoffs. Mentions that communication volume differs by ZeRO stage.
\end{itemize}

\textbf{Common follow-ups:} ``What changes if we use sequence length 32K instead of 2K?'' (Activation memory explodes due to $O(n^2)$ attention, unless Flash Attention is used.) ``How many A100s do you need to train this model?'' ``What's the difference between ZeRO-1, ZeRO-2, and ZeRO-3?''
\end{interviewq}
...
\textbf{Strong answer:} RoPE (Rotary Position Embedding) encodes position by rotating query and key vectors in 2D subspaces. Each pair of dimensions $(2i, 2i+1)$ gets rotated by angle $m\theta_i$ where $m$ is the position and $\theta_i = 10000^{-2i/d}$ controls the frequency.
...
\textbf{What the interviewer is testing:} Whether you understand how modern LLMs handle position, the extrapolation problem, and why the field moved away from learned embeddings.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Explains rotation concept, relative position property, and advantages over learned embeddings. Knows LLaMA and most modern LLMs use RoPE.
    \item \textbf{L6:} Can explain the frequency design ($\theta_i$ values), why different dimensions capture different position scales (low dimensions = fine-grained, high dimensions = coarse). Knows Position Interpolation vs NTK-aware scaling for context extension.
    \item \textbf{L7:} Understands why extrapolation fails (OOD rotation angles at unseen positions), can compare RoPE vs ALiBi tradeoffs (ALiBi extrapolates better but RoPE is more expressive). Knows about the ``lost in the middle'' phenomenon and how position encoding relates to it.
\end{itemize}

\textbf{Common follow-ups:} ``How would you extend a RoPE-based model from 4K to 128K context?'' ``Compare RoPE vs ALiBi.'' ``Why does naive extrapolation fail?''
\end{interviewq}
...
\textbf{Strong answer:} These are three points on the spectrum of KV head sharing:
...
\begin{itemize}
    ...
    \item \textbf{Extreme latency constraint, small model}: MQA. Used in PaLM, Falcon. Acceptable when model capacity is large enough to compensate.
\end{itemize}
...
\textbf{What the interviewer is testing:} Understanding of inference optimization, KV cache memory math, and quality-speed trade-offs in serving.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correctly explains all three variants and that GQA is the modern default. Knows it reduces KV cache.
    \item \textbf{L6:} Can calculate KV cache sizes for specific models. Understands that GQA also improves inference throughput by reducing memory bandwidth for KV reads. Knows how to convert MHA $\to$ GQA (average KV heads within groups, fine-tune).
    \item \textbf{L7:} Understands the interaction with PagedAttention (smaller KV per page = better memory utilization). Knows that GQA's benefit is proportional to the decode-to-prefill ratio of the workload. Can reason about KV cache quantization as an orthogonal optimization.
\end{itemize}

\textbf{Common follow-ups:} ``How would you convert an existing MHA model to GQA?'' ``What's the impact on training speed vs inference speed?'' ``How does GQA interact with KV cache quantization?''
\end{interviewq}
...
\textbf{Strong answer:} The dot product of two random vectors with independent entries of mean 0 and variance 1 has variance equal to $d_k$ (the dimension). As $d_k$ grows, dot products become large in magnitude, pushing softmax into saturation regions where:
...
\textbf{What the interviewer is testing:} Can you reason about mathematical properties from first principles? Do you understand the interaction between dot products, softmax, and gradients?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Explains variance growth and softmax saturation. Gets the $\sqrt{d_k}$ normalization right.
    \item \textbf{L6:} Can derive the variance formula. Notes that this is specific to dot-product attention (additive attention doesn't need scaling). Connects to temperature-scaled softmax.
    \item \textbf{L7:} Discusses that in practice, Q and K aren't random --- learned correlations can still cause saturation even with scaling. Mentions QK-Norm (normalizing Q and K before dot product) as an alternative used in some architectures. Notes that the $\sqrt{d_k}$ scaling is equivalent to initializing $\mW^Q, \mW^K$ with std $\sim 1/\sqrt{d_k}$.
\end{itemize}

\textbf{Common follow-ups:} ``What if we used additive attention --- would we need scaling?'' ``What's the connection to temperature in softmax?'' ``How does this interact with mixed precision training?''
\end{interviewq}
...
\textbf{Strong answer:} A useful rule of thumb: for a Transformer, forward pass FLOPs $\approx 2 \times$ parameters $\times$ tokens. Backward is $\approx 2\times$ forward. So total training FLOPs per token $\approx 6P$.
...
\textbf{What the interviewer is testing:} Ability to do back-of-envelope compute calculations. Understanding of where FLOPs go in a Transformer.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Knows the $2P$ forward, $6P$ training rule. Gets the right order of magnitude.
    \item \textbf{L6:} Can derive per-layer breakdown. Knows the attention term ($4n^2d$) vs projection term ($24nd^2$). Can identify when attention FLOPs dominate (when $n > 6d$, roughly $n > 24K$ for $d=4096$).
    \item \textbf{L7:} Can convert FLOPs to wall-clock time given GPU specs (A100 = 312 TFLOPS BF16). Knows MFU (Model FLOPs Utilization) is typically 40--60\%. Can estimate total training cost: Chinchilla-optimal for 7B $\approx$ 140B tokens $\rightarrow$ $6 \times 7B \times 140B = 5.88 \times 10^{21}$ FLOPs $\approx$ 30K A100-hours.
\end{itemize}

\textbf{Common follow-ups:} ``How does this change at 128K context?'' ``What's the training cost in GPU-hours?'' ``Where does the $6P$ come from?'' (2P forward + 4P backward.)
\end{interviewq}
...
\textbf{What the interviewer is testing:} Can you size LLM serving infrastructure? Do you understand the KV cache bottleneck?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correctly applies the formula, gets the right answer. Knows KV cache is the memory bottleneck for long context.
    \item \textbf{L6:} Discusses mitigation strategies: KV cache quantization (INT8 halves it), PagedAttention (reduces fragmentation waste), GQA (already reflected in LLaMA 70B's 8 KV heads).
    \item \textbf{L7:} Reasons about system-level implications: at 128K with 42 GB KV cache per sequence, batch size is effectively 1 on a single 80 GB A100 (after model weights). Proposes disaggregated prefill/decode, or prefix caching for shared system prompts. Mentions H100 with 188 GB HBM3 or multi-GPU serving as solutions.
\end{itemize}

\textbf{Common follow-ups:} ``How would you reduce this KV cache size?'' ``What batch size can you support on 8xA100?'' ``How does prefix caching help for chatbot workloads?''
\end{interviewq}

\begin{interviewq}
\textbf{Q: Why is self-attention permutation equivariant? Why does this matter for Transformers?}

\textbf{Difficulty:} L5 \quad \textbf{Type:} First Principles \quad \textbf{Frequency:} \freqmed{}

\textbf{Strong answer:} Self-attention computes each output as a weighted combination of all values, where weights depend on query-key similarity. If you permute the input positions, every output gets the same weights (just reordered) because:
...
\textbf{What the interviewer is testing:} Understanding of fundamental Transformer properties and why architectural choices (positional encodings) are necessary.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correctly explains equivariance, distinguishes from invariance, and connects to the need for positional encodings.
    \item \textbf{L6:} Notes that this is actually a \textit{feature} for set-input tasks (point clouds, graph nodes). Explains how causal masking breaks pure equivariance but doesn't fully encode absolute position.
    \item \textbf{L7:} Discusses implications for architectural choices: why ViT needs CLS tokens, why BERT's position embeddings are critical for NLU tasks, how DeepSets/Set Transformer exploit this for set functions. Relates to group theory: attention respects the symmetric group $S_n$.
\end{itemize}

\textbf{Common follow-ups:} ``What's the difference between equivariant and invariant?'' ``When is permutation equivariance desirable?'' ``Does causal masking break equivariance?''
\end{interviewq}
...
\textbf{The case for Post-norm}: When it converges, it can achieve slightly better final quality. Some evidence suggests the LayerNorm on the residual stream provides a beneficial regularization effect. This is why some models (PaLM) use Post-norm with careful initialization (e.g., the $1/\sqrt{2N}$ scaling from DeepNet).
...
\textbf{What the interviewer is testing:} Understanding of training stability, gradient flow, and why architectural defaults exist.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Knows pre-norm is the modern default. Can explain the gradient flow argument.
    \item \textbf{L6:} Understands the quality gap (post-norm can be slightly better when stable). Knows about DeepNet-style initialization that makes post-norm viable for deep models. Can connect to the broader principle of ``clean residual paths.''
    \item \textbf{L7:} Discusses the interaction with other stabilization techniques (QK-Norm, $\mu$P). Notes that the ``pre-norm is always better'' narrative is oversimplified --- some recent work shows post-norm with proper initialization can match or exceed pre-norm.
\end{itemize}

\textbf{Common follow-ups:} ``What happens to gradient magnitudes in very deep post-norm networks?'' ``What's the connection to ResNet's skip connections?'' ``How does the $1/\sqrt{2N}$ scaling work?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Debugging methodology for LLM behavior, understanding of context length limitations, knowledge of position encoding techniques.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Identifies position encoding as the culprit. Suggests fine-tuning on longer data.
    \item \textbf{L6:} Knows Position Interpolation vs NTK-aware scaling vs YaRN. Can propose a specific fix and estimate the cost (1K fine-tuning steps).
    \item \textbf{L7:} Considers secondary issues: attention sinks (first few tokens get disproportionate attention at long context), lost-in-the-middle phenomenon, and whether the model's effective context window matches its theoretical one. Proposes evaluation methodology (needle-in-a-haystack test at various depths).
\end{itemize}

\textbf{Common follow-ups:} ``What's the difference between PI and NTK-aware scaling?'' ``How would you evaluate if the fix worked?'' ``What's the 'lost in the middle' problem?''
\end{interviewq}
...
\textbf{Strong answer:} Standard attention at 1M tokens is infeasible: the attention matrix alone would be $10^{12}$ elements ($\sim$2 TB in FP16). Even Flash Attention can't help --- the $O(n^2)$ compute is $\sim$$10^{15}$ FLOPs per layer.
...
\textbf{What the interviewer is testing:} Can you design novel architectures under extreme constraints? Do you understand the compute/memory/quality tradeoffs?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Identifies the $O(n^2)$ problem and suggests sparse attention or chunking.
    \item \textbf{L6:} Proposes a concrete architecture (hierarchical attention with local + global). Considers Ring Attention for distributed approach. Mentions SSMs as an alternative.
    \item \textbf{L7:} Reasons about what tasks actually need 1M-token attention vs what can be solved with retrieval. Considers hybrid architectures (Transformer + Mamba layers). Proposes evaluation criteria: does the task require precise retrieval from arbitrary positions (favors attention) or sequential summarization (favors SSMs)?
\end{itemize}

\textbf{Common follow-ups:} ``When would RAG be better than long-context?'' ``Compare this to Mamba for document understanding.'' ``How would you evaluate if the model actually uses long-range context?''
\end{interviewq}
...
\begin{enumerate}
    ...
    \item \textbf{Vocabulary size 50K $\to$ 32K}: LLaMA uses SentencePiece BPE vs GPT-2's BPE. Smaller vocab = smaller embedding table, but this varies by model.
\end{enumerate}
...
\textbf{What the interviewer is testing:} Awareness of modern LLM architecture evolution and understanding of \textit{why} each design choice was made.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Names at least 4 of the 6 changes with correct motivations.
    \item \textbf{L6:} Explains the \textit{why} behind each change (stability, efficiency, quality). Can discuss the SwiGLU expansion factor adjustment. Knows that these choices are now standard across Mistral, Qwen, Yi, etc.
    \item \textbf{L7:} Discusses what might change next: QK-Norm for extreme scale, mixture of experts (Mixtral), deeper-and-narrower vs wider-and-shallower tradeoffs, potential move to post-norm with proper initialization, $\mu$P for hyperparameter transfer across scales.
\end{itemize}

\textbf{Common follow-ups:} ``Why RMSNorm specifically rather than LayerNorm?'' ``What's the SwiGLU expansion factor and why?'' ``What do you expect to change next?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Awareness of alternatives to attention, ability to reason about architectural tradeoffs, understanding of where each architecture excels.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Knows Mamba is $O(n)$ vs Transformer's $O(n^2)$ attention, and that Mamba has a fixed-size state. Can name a basic tradeoff.
    \item \textbf{L6:} Understands \textit{why} Mamba is weaker at retrieval (information must be compressed into a fixed-size state, causing information bottleneck). Knows about hybrid architectures. Can reason about when the efficiency gains actually matter in practice.
    \item \textbf{L7:} Discusses the theoretical expressiveness gap (attention can implement arbitrary lookups; SSMs are limited by state dimension). Mentions that for language modeling perplexity, Mamba matches Transformers at same compute budget, but downstream task performance (especially retrieval-heavy) diverges. Can reason about whether hybrid architectures can get the best of both worlds.
\end{itemize}

\textbf{Common follow-ups:} ``Why can't Mamba do retrieval as well as attention?'' ``What's the selective mechanism in Mamba?'' ``How do hybrid architectures like Jamba work?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Deep understanding of attention mechanics in practice, awareness of attention sink literature, debugging LLM behavior.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Recognizes this as a common pattern. Knows it's related to softmax normalization.
    \item \textbf{L6:} Explains the ``rest state'' interpretation. Distinguishes between healthy sink behavior (some heads) vs pathological (all heads). Mentions StreamingLLM.
    \item \textbf{L7:} Connects to KV cache eviction strategies (must preserve sink tokens). Discusses implications for attention head pruning (sink heads might be prunable). Relates to the broader question of what attention heads actually compute and how to interpret them.
\end{itemize}

\textbf{Common follow-ups:} ``How does StreamingLLM use attention sinks?'' ``Can you prune attention heads that only attend to sinks?'' ``What does this tell us about the model's residual stream?''
\end{interviewq}
...
\textbf{Crossover point}: Attention and FFN FLOPs are equal when $n = 2d$. For $d=4096$, this is $n \approx 8K$.
...
\textbf{What the interviewer is testing:} Quantitative reasoning about Transformer compute. Can you identify which component to optimize?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Knows attention is $O(n^2 d)$ and FFN is $O(nd^2)$. Can state which dominates at long vs short sequences.
    \item \textbf{L6:} Can derive the crossover point ($n \approx 2d$). Gives concrete numbers for specific models and sequence lengths.
    \item \textbf{L7:} Distinguishes training (all tokens processed in parallel, attention usually subdominant) from inference (decode phase: $n=1$ per step, FFN totally dominates for weight loading). Notes that Flash Attention changes the practical bottleneck even when attention theoretically dominates (by making attention compute-bound instead of memory-bound).
\end{itemize}

\textbf{Common follow-ups:} ``How does this analysis change for inference vs training?'' ``Where should you spend your optimization effort for a 128K-context model?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Can you adapt architectures for specific constraints? Understanding of streaming vs batch processing tradeoffs.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Identifies the need for causal masking and bounded context.
    \item \textbf{L6:} Proposes sliding window attention, mentions StreamingLLM. Discusses latency-quality tradeoff of window size.
    \item \textbf{L7:} Proposes a complete system: chunked processing with state carryover, specific window sizes with justification, consideration of Mamba/SSM for streaming, and evaluation methodology (WER vs latency Pareto curve for ASR).
\end{itemize}

\textbf{Common follow-ups:} ``What's the latency-quality tradeoff of window size?'' ``How would you evaluate streaming vs batch quality?'' ``What about tasks that need full-document context?''
\end{interviewq}
...
\begin{itemize}
    ...
    \item \textbf{$n > 128K$}: Seriously consider linear attention, sparse attention, or SSMs. Flash Attention's $O(n^2)$ compute becomes prohibitive ($128K^2 \approx 16B$ operations per layer per head).
...
\textbf{What the interviewer is testing:} Understanding that Flash Attention and linear attention solve different problems (hardware efficiency vs algorithmic complexity).

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correctly states that Flash Attention is exact while linear attention is approximate. Knows Flash is $O(n^2)$ compute.
    \item \textbf{L6:} Explains the kernel trick behind linear attention. Identifies the streaming use case. Quantifies the crossover point ($\sim$32K).
    \item \textbf{L7:} Discusses recent work combining both (e.g., Hedgehog: learning the kernel $\phi$ to approximate softmax more closely). Notes that in practice, Mamba/SSMs have largely displaced linear attention for the ``efficient long-context'' niche because they achieve better quality at similar efficiency.
\end{itemize}

\textbf{Common follow-ups:} ``What's the kernel trick in linear attention?'' ``Why have SSMs been more successful than linear attention?'' ``Can you combine Flash Attention with sparse attention patterns?''
\end{interviewq}
