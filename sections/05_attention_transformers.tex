...
\textbf{Luong Attention.} Introduced by Luong et al.\ (2015) as a simpler and faster alternative to Bahdanau attention for sequence-to-sequence models. The core idea: compute alignment scores using multiplicative (bilinear) functions instead of an additive MLP.

\begin{mathresult}{Luong Attention --- Three Score Functions}
\begin{align}
\text{Dot:} \quad e_i &= \vh_t^T \bar{\vh}_s \label{eq:luong_dot} \\
\text{General:} \quad e_i &= \vh_t^T \mW_a \bar{\vh}_s \label{eq:luong_general} \\
\text{Concat:} \quad e_i &= \vv_a^T \tanh(\mW_a [\vh_t ; \bar{\vh}_s]) \label{eq:luong_concat}
\end{align}
where $\vh_t$ is the current decoder hidden state and $\bar{\vh}_s$ are encoder hidden states.
\end{mathresult}

\textbf{Key difference from Bahdanau}: Bahdanau uses the \emph{previous} decoder hidden state $\vh_{t-1}$ to compute attention (because the context feeds into the current step's RNN input). Luong uses the \emph{current} top-layer decoder hidden state $\vh_t$, which is simpler to implement and avoids the recurrence dependency in the attention computation.

\textbf{Properties}:
\begin{itemize}
    \item Simpler, faster (no nonlinearity in dot/general variants)
    \item Dot product requires $d_q = d_k$
    \item General form adds expressivity with $\mW_a$
    \item Concat variant is functionally equivalent to Bahdanau (additive) attention
\end{itemize}

\textbf{Global vs.\ Local Attention.} Luong also introduced a \textbf{local attention} variant:
\begin{itemize}
    \item \textbf{Global attention}: Computes alignment over \emph{all} source positions (same as standard soft attention). Cost is $O(n)$ per decoder step.
    \item \textbf{Local attention}: Predicts an aligned position $p_t$ and attends only to a window $[p_t - D, p_t + D]$. This is $O(D)$ per step regardless of source length---a precursor to the sliding-window attention used in modern efficient Transformers (Longformer, Mistral).
\end{itemize}

\textbf{When to use additive vs.\ multiplicative?} Multiplicative (dot/general) is faster in practice because the score computation reduces to a single matrix multiply, which is heavily optimized on GPUs. Additive attention is more expressive for small $d_k$ because the $\tanh$ nonlinearity can learn complex alignment patterns. For large $d_k$ (as in Transformers), the expressivity gap vanishes and the speed advantage of multiplicative attention dominates. This is why scaled dot-product attention became the standard.

\textbf{Historical significance}: Luong attention (2015) and Bahdanau attention (2014) together established the attention vocabulary---queries, keys, values, alignment scores---that Vaswani et al.\ (2017) generalized into self-attention and the Transformer. The progression from additive $\to$ multiplicative $\to$ scaled dot-product is a direct lineage.
...
\textbf{Learned Positional Embedding.} Instead of using a fixed function of position, maintain a trainable embedding matrix $\mP \in \R^{L_{\max} \times d_{\text{model}}}$ where each row corresponds to a position index. The position encoding for token at position $t$ is simply:

\begin{equation}
\mE_{pos} \in \R^{L_{max} \times d}, \quad \text{PE}(t) = \mP[t, :]
\end{equation}

This embedding is added to the token embedding exactly as with sinusoidal encodings: $\vx_t = \text{TokenEmbed}(w_t) + \mP[t, :]$. The matrix $\mP$ is initialized randomly and learned end-to-end via backpropagation.
...
\begin{itemize}
    ...
    \item \badgeLsix{L6} Can explain the online softmax trick, knows the SRAM/HBM sizes on A100, understands the arithmetic intensity shift from memory-bound to compute-bound. Mentions Flash Attention 2 improvements (better parallelism, causal masking optimization).
    \item \badgeLseven{L7} Discusses implications for system design: Flash Attention changes which operations are the bottleneck (now FFN, not attention). Knows that FA doesn't help for decode phase (which is memory-bound for a different reason: loading model weights, not attention matrix). Can reason about when IO-aware algorithms matter beyond attention.
\end{itemize}
...
\followups{``What changes if we use sequence length 32K instead of 2K?'' (Activation memory explodes due to $O(n^2)$ attention, unless Flash Attention is used.) ``How many A100s do you need to train this model?'' ``What's the difference between ZeRO-1, ZeRO-2, and ZeRO-3?''}
\end{interviewq}
...
\begin{itemize}
    ...
    \item \badgeLseven{L7} Can convert FLOPs to wall-clock time given GPU specs (A100 = 312 TFLOPS BF16). Knows MFU (Model FLOPs Utilization) is typically 40--60\%. Can estimate total training cost: Chinchilla-optimal for 7B $\approx$ 140B tokens $\rightarrow$ $6 \times 7B \times 140B = 5.88 \times 10^{21}$ FLOPs $\approx$ 30K A100-hours.
\end{itemize}
...
\followups{``How would you reduce this KV cache size?'' ``What batch size can you support on 8xA100?'' ``How does prefix caching help for chatbot workloads?''}
\end{interviewq}
...
\begin{itemize}
    ...
    \item \badgeLseven{L7} Discusses what might change next: QK-Norm for extreme scale, mixture of experts (Mixtral), deeper-and-narrower vs wider-and-shallower tradeoffs, potential move to post-norm with proper initialization, $\mu$P for hyperparameter transfer across scales.
\end{itemize}

\followups{``Why RMSNorm specifically rather than LayerNorm?'' ``What's the SwiGLU expansion factor and why?'' ``What do you expect to change next?''}
\end{interviewq}
...
\begin{itemize}
    ...
    \item \badgeLseven{L7} Distinguishes training (all tokens processed in parallel, attention usually subdominant) from inference (decode phase: $n=1$ per step, FFN totally dominates for weight loading). Notes that Flash Attention changes the practical bottleneck even when attention theoretically dominates (by making attention compute-bound instead of memory-bound).
\end{itemize}
...
\begin{itemize}
    ...
    \item \badgeLseven{L7} Discusses recent work combining both (e.g., Hedgehog: learning the kernel $\phi$ to approximate softmax more closely). Notes that in practice, Mamba/SSMs have largely displaced linear attention for the ``efficient long-context'' niche because they achieve better quality at similar efficiency.
\end{itemize}

\followups{``What's the kernel trick in linear attention?'' ``Why have SSMs been more successful than linear attention?'' ``Can you combine Flash Attention with sparse attention patterns?''}
\end{interviewq}
