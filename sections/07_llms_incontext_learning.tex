...
\item \textbf{Chain-of-thought reasoning}: only effective in models above
  $\sim$60B parameters (PaLM, GPT-3.5+).
...
Dai et al.\ (2023) showed a formal connection between in-context learning
and gradient descent.  The attention mechanism in a transformer performing ICL
produces updates equivalent to what a single step of gradient descent on a
linear model would produce, using the demonstration examples as training data.
In this view, the model's forward pass \emph{implicitly} fine-tunes an
internal representation---it just does so without explicitly computing
gradients or updating weights.  This explains why more demonstrations
generally help, why example quality matters, and why ICL performance
improves with model scale (more capacity for implicit optimization).
\end{keyinsight}
...
\item Requires sufficient model scale to be effective ($>$60B parameters in
  the original findings, though smaller instruction-tuned models have narrowed
  this gap).
\end{itemize}
...
Examples & GPT-3 $\to$ GPT-4 & o1/o3, Best-of-N, tree search \\
\bottomrule
\end{tabularx}
...
For a 70B model with 128K context in float16, this can exceed 40\,GB.
...
\item \textbf{Retrieval-augmented approaches:} Rather than placing all
  ...
  Covered in detail in Chapter~\ref{chap:llms_icl} on RAG (Chapter 9).
\end{itemize}
...
\midrule
Switch Transformer & 1.6T & 100B & 128 (top-1)
...
Dai et al.\ (2023) formally showed that transformer attention during ICL
produces updates equivalent to a single gradient descent step on the
demonstrations---so ICL is effectively \textbf{implicit fine-tuning} within
the forward pass.
...
\item \badgeLseven{L7}: Discusses the formal relationship between ICL and
  gradient descent, the role of induction heads in mechanistic
  interpretability, and the open question of whether ICL implements true
  learning or sophisticated pattern matching.
\end{itemize}
...
The mechanism involves: (1)~the model generates an extended internal reasoning
chain, potentially with backtracking and self-correction; (2)~a Process
Reward Model (PRM) evaluates each step, enabling search over reasoning paths;
(3)~best-of-N sampling or tree search selects the highest-quality solution.
...
\item Not mentioning process reward models or verification.
\item Saying ``it just generates more tokens'' without explaining the
...
\item \badgeLsix{L6}: Discusses PRMs, best-of-N, and tree search as
  mechanisms; explains the cost tradeoff (per-query vs.\ fixed training
  cost); names specific models (o1, o3).
