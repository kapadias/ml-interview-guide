...
of the noise pairs toward $-\infty$ (low similarity).  Mikolov et al.\ showed
that this is equivalent to maximizing a lower bound on the mutual information
between center and context words, and that it approximates the full softmax
gradient.
...
\]
where $s_\ell$ are learned softmax-normalized weights (learned during
fine-tuning on the downstream task) and $\gamma$ is a scalar scaling factor.
