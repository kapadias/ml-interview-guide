...
is decomposed into the longest matching subword units.  For example, a rare word like
``unparalleledly'' might tokenize as \texttt{["un", "parallel", "ed", "ly"]}.
...
\paragraph{Used by:} T5, XLNet, LLaMA (via SentencePiece), ALBERT.
...
\emph{language agnosticism}: it treats the input as a raw byte stream with no
pre-tokenization (no whitespace splitting, no language-specific rules).  This makes it
...
\midrule
$\sim$32K  & Longer   & 130M (at $d{=}4096$) & GPT-2, LLaMA 1 \\
$\sim$50K  & Moderate & 200M (at $d{=}4096$) & GPT-3, BERT \\
$\sim$100K & Shorter  & 400M (at $d{=}4096$) & GPT-4, LLaMA 3 \\
$\sim$150K & Shortest & 600M (at $d{=}4096$) & Gemini \\
\bottomrule
\end{tabularx}
...
token generated.  For a 70B parameter model with 128K context, the KV-cache alone can
exceed 40\,GB---larger than the model weights themselves.  This makes LLM inference
...
\begin{tikzpicture}[
    ...
    sublbl/.style={font=\scriptsize\sffamily, text=gray!70!black},
    arr/.style={-{Stealth[length=4pt]}, thick, deepblue!60},
...
\begin{itemize}
    ...
    \item Result: GPU utilization approaches 100\%, throughput increases 2--5$\times$
...
one.  For a 70B parameter model, generating a single token takes $\sim$30\,ms on an A100
(reading 140\,GB of weights at $\sim$2\,TB/s memory bandwidth).  With speculative
...
\]
where $c_{\text{draft}} / c_{\text{target}} \ll 1$.  With $k = 5$ and 80\% acceptance
rate, this yields roughly $2.5\times$ speedup.
...
extremely demanding---let me work through the key design decisions.

\textbf{Model selection and optimization:}  Start with the smallest model that meets
quality requirements.  A 7B model with INT4 quantization (AWQ or GPTQ) fits on a single
A100/H100.  TTFT is dominated by prefill: for a 1K-token prompt on a 7B INT4 model,
prefill takes $\sim$50ms on H100.  If a 70B model is required, use GQA + INT4
quantization + tensor parallelism across 4 GPUs.

\textbf{Batching:}  Continuous batching is essential.  With iteration-level scheduling,
the system can sustain much higher throughput than static batching.  A single H100 can
serve a 7B INT4 model at $\sim$500 QPS with 1K-token prompts.  So 10K QPS requires
$\sim$20 GPUs.
...
\textbf{Capacity planning:}  With 10K QPS and 500ms TTFT, at any given moment there are
$\sim$5,000 concurrent requests in prefill.  Each request's KV-cache (7B model, 1K
context, INT4 KV) is $\sim$64MB.  Total KV-cache memory: $\sim$320GB across the cluster.
PagedAttention is essential to manage this efficiently.
