...
to the bigram, and if that is zero, to the unigram.  Stupid backoff does not
produce normalized probabilities but works well at web scale.
...
English text, state-of-the-art n-gram models achieve perplexities around
50--100; modern neural LMs achieve 10--30 on standard benchmarks.
\end{keyinsight}
...
achieve $80$.  GPT-2 achieved perplexity $\approx 29$ on Penn Treebank.
...
For decades, HMMs were the dominant approach to sequence labeling tasks like
part-of-speech tagging and named entity recognition, and they remain the
standard introduction to probabilistic sequence modeling.
...
algorithm:
\begin{enumerate}
    \item \textbf{E-step:} Using the current parameters and both the forward
    ($\alpha$) and backward ($\beta$) variables, compute the expected counts
    of transitions and emissions.  Define:
    \[
    \gamma_t(i) = P(y_t = s_i \given \vx), \qquad
    \xi_t(i,j) = P(y_t = s_i, y_{t+1} = s_j \given \vx)
    \]

    \item \textbf{M-step:} Re-estimate parameters using expected counts:
    \begin{align}
    \hat{a}_{ij} &= \frac{\sum_{t=1}^{T-1} \xi_t(i,j)}{\sum_{t=1}^{T-1} \gamma_t(i)} \label{eq:bw_transition} \\[4pt]
    \hat{b}_j(o) &= \frac{\sum_{t : x_t = o} \gamma_t(j)}{\sum_{t=1}^{T} \gamma_t(j)} \label{eq:bw_emission}
    \end{align}

    \item Repeat until convergence.  Baum-Welch guarantees monotonic increase
    of the data likelihood (as a consequence of the EM convergence theorem).
\end{enumerate}
...
which are computed by the forward-backward algorithm for CRFs.
...
\begin{itemize}
    ...
    $a(i \given j, l_e, l_f)$.
    \item \textbf{IBM Models 3--5:} Add fertility (one English word can generate
    multiple foreign words), distortion, and more complex alignment structures.
\end{itemize}
...
Second, CRFs avoid the \emph{label bias problem} that affects Maximum Entropy
Markov Models (MEMMs), which are locally normalized.  CRFs are globally
normalized via the partition function $Z(\vx)$, ensuring that all paths
