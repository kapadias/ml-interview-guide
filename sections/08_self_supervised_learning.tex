...
\begin{interviewq}
\textbf{Q: Compare CLIP's approach to vision-language alignment with methods like LLaVA or Flamingo. What are the tradeoffs?}

\textbf{Difficulty:} L6 \quad \textbf{Type:} Trade-off \quad \textbf{Frequency:} \freqmed{}

\textbf{Strong answer:} CLIP, LLaVA, and Flamingo represent three generations of vision-language models with fundamentally different design philosophies.

\textit{CLIP: Contrastive alignment.} CLIP trains separate image and text encoders to produce aligned embeddings via contrastive loss. Strengths: produces a shared embedding space enabling zero-shot classification, retrieval, and similarity search; embeddings are reusable and efficient (compute once, use many times). Weaknesses: cannot generate text, cannot perform complex reasoning over image content, and the fixed-size embedding discards fine-grained spatial information.

\textit{LLaVA: Adapter-based VLM.} LLaVA connects a frozen CLIP vision encoder to a frozen LLM via a trained projection layer, then fine-tunes the LLM on visual instruction data. Strengths: leverages strong pre-trained components, enables open-ended visual question answering and reasoning, relatively cheap to train (only the adapter + LLM fine-tuning). Weaknesses: vision understanding is bottlenecked by CLIP's representation, cannot perceive details CLIP didn't encode, and requires autoregressive generation (slow for simple tasks).

\textit{Flamingo: Cross-attention VLM.} Flamingo injects visual features into a frozen LLM via gated cross-attention layers, trained on interleaved image-text data. Strengths: handles multiple images in context, strong few-shot performance, preserves the LLM's language capabilities. Weaknesses: expensive to train (requires large-scale interleaved data), cross-attention adds latency.

Key tradeoff: CLIP gives you efficient embeddings but no reasoning; LLaVA/Flamingo give reasoning but at higher inference cost. For retrieval/search, CLIP is unbeatable. For VQA/reasoning, you need a generative VLM.

\textbf{Red flags} (what NOT to say):
\begin{itemize}
    \item Treating CLIP as obsolete---it's still the foundation of most VLMs
    \item Not understanding that LLaVA uses CLIP as a component
    \item Confusing adapter-based (LLaVA) with cross-attention (Flamingo) approaches
\end{itemize}

\textbf{What the interviewer is testing:} Understanding of the vision-language model ecosystem and architectural tradeoffs.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Describes each model correctly and its primary use case
    \item \textbf{L6:} Articulates the embedding vs.\ generative tradeoff, explains architectural differences
    \item \textbf{L7:} Discusses native multimodal models (Gemini) that train vision and language jointly, and why the ``connect pre-trained components'' approach may have a ceiling
\end{itemize}

\textbf{Common follow-ups:} ``How does SigLIP improve on CLIP?'' ``Why does LLaVA use a simple linear projection rather than cross-attention?'' ``What are the limitations of CLIP-based vision for VLMs?''
\end{interviewq}