...
\textbf{Common follow-ups:} ``How do you choose LoRA rank?'' (Start with 8-16 for most tasks. Increase to 64 for complex tasks like code generation. Higher rank = more capacity but more parameters.) ``Can you combine LoRA adapters?'' (Yes, adapter merging and task arithmetic work well for multi-task serving.)
\end{interviewq}
...
\textbf{What the interviewer is testing:} Understanding of the PEFT landscape and practical decision-making.

\textbf{A:}

\begin{itemize}
    \item \textbf{Full fine-tuning}: Best quality, but requires the most memory and compute. Use when: you have abundant data, quality is paramount, and you can afford multi-GPU training. Typical scenario: building a production model that justifies the infrastructure cost.

    \item \textbf{LoRA}: Best balance of quality and efficiency. Trains 0.1-1\% of parameters, fits on a single GPU. Use when: fine-tuning LLMs with moderate data (1K-100K examples), need to serve multiple task-specific models (swap adapters), or GPU budget is limited.

    \item \textbf{Prompt tuning}: Most parameter-efficient ($<$0.01\% parameters), but lowest quality. Use when: you want task-specific behavior without any weight modifications, or when you're serving hundreds of tasks from one base model and can't afford per-task adapters.
\end{itemize}

\textbf{Decision rule}: Default to LoRA. Upgrade to full fine-tuning only if LoRA quality is insufficient. Use prompt tuning only for very lightweight customization.

\textbf{Common follow-ups:} ``What about adapter layers vs LoRA?'' (Similar performance, but LoRA has no inference overhead when merged, while adapters add latency.) ``Does LoRA work for pre-training?'' (Not typically --- LoRA is designed for adaptation from an existing checkpoint, not learning from scratch.)
\end{interviewq}

\begin{interviewq}
\textbf{Q: You fine-tuned a model and it performs well on your test set but poorly in production. What happened?}

\textbf{What the interviewer is testing:} Understanding of distribution shift, catastrophic forgetting, and practical fine-tuning pitfalls.

\textbf{A:} Several likely causes:

\begin{enumerate}
    \item \textbf{Catastrophic forgetting}: Aggressive fine-tuning destroyed the model's general capabilities. The model overfits to the fine-tuning distribution and can't handle the broader range of production inputs. \textit{Fix}: Lower learning rate, fewer epochs, use LoRA instead of full fine-tuning.

    \item \textbf{Distribution shift}: Test set doesn't match production distribution. Maybe test data was curated/clean but production data has typos, slang, or edge cases. \textit{Fix}: Evaluate on production-like data before deploying.

    \item \textbf{Data leakage}: Test set overlaps with training data, inflating offline metrics. \textit{Fix}: Strict temporal splits, check for near-duplicates.

    \item \textbf{Evaluation metric mismatch}: High accuracy on test set doesn't mean the model handles the failure modes that matter in production. \textit{Fix}: Evaluate on adversarial/edge cases, not just IID test data.
\end{enumerate}

\textbf{Common follow-ups:} ``How do you prevent catastrophic forgetting?'' (LoRA naturally prevents it by keeping base weights frozen. For full fine-tuning: low LR, mix in pre-training data, use EWC or similar regularization.)
\end{interviewq}