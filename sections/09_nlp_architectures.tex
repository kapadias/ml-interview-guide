...
\begin{equation}
\mathcal{L}_{ELECTRA} = \sum_{i=1}^{n} -\mathbb{1}[x_i^{corrupt} = x_i^{original}] \log D(x_i) - \mathbb{1}[x_i^{corrupt} \neq x_i^{original}] \log(1 - D(x_i))
\end{equation}

where $D(x_i) \in (0, 1)$ is the discriminator's predicted probability that token $i$ is \emph{original} (i.e., was not replaced by the generator), following the paper's convention.
