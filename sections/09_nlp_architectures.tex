...
\textbf{Strong answer:} The choice maps to three factors: task type, data availability, and latency requirements.
...
\textbf{What the interviewer is testing:} Practical architecture selection based on constraints, not just theoretical knowledge.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correctly maps task types to architecture families
    \item \textbf{L6:} Discusses the scale-dependent nature of the tradeoff, mentions cost and latency
    \item \textbf{L7:} Explains why decoder-only dominates at scale (training signal density, interface universality, simpler infrastructure), discusses when encoder models are still the right choice in production
\end{itemize}

\textbf{Common follow-ups:} ``Would you use GPT-4 for production classification?'' ``Why did T5's encoder-decoder approach not scale as well as decoder-only?'' ``Can you use a decoder for embedding/retrieval tasks?''
\end{interviewq}
...
\textbf{Strong answer:} Fluent but factually incorrect output is the hallmark of ``hallucination''---the model generates plausible-sounding text that isn't grounded in reality. I would diagnose along three axes.
...
\textbf{What the interviewer is testing:} Systematic debugging of LLM behavior, understanding of hallucination causes and mitigations.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Identifies hallucination as the issue, suggests RAG
    \item \textbf{L6:} Provides structured diagnosis (data, decoding, knowledge boundary), multiple mitigations
    \item \textbf{L7:} Discusses the fundamental tension between fluency and factuality objectives, mentions process reward models for step-by-step verification, addresses evaluation of factuality at scale
\end{itemize}

\textbf{Common follow-ups:} ``How do you measure hallucination rate?'' ``When does RAG fail?'' ``How does RLHF contribute to hallucination?''
\end{interviewq}
...
\textbf{Strong answer:} LLM inference has two phases: prefill (process the prompt) and decode (generate tokens one at a time). I'll estimate both.
...
\textbf{What the interviewer is testing:} Understanding of LLM inference bottlenecks and ability to do back-of-envelope calculations.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Gets the right order of magnitude (seconds, not minutes or milliseconds)
    \item \textbf{L6:} Distinguishes prefill (compute-bound) from decode (memory-bound), uses memory bandwidth for per-token estimate
    \item \textbf{L7:} Discusses how batching changes the arithmetic (decode becomes compute-bound at batch size $\sim$64+), mentions quantization (INT4 halves per-token latency), discusses KV cache memory as the context-length bottleneck
\end{itemize}

\textbf{Common follow-ups:} ``How does INT4 quantization change the numbers?'' ``What batch size makes decode compute-bound?'' ``How much memory does the KV cache need for 4K context?''
\end{interviewq}
...
\textbf{Strong answer:} In-context learning (ICL) is one of the most surprising emergent properties of large LLMs. The model performs a new task given only a few examples in the prompt---no parameter updates. The leading mechanistic explanations are:
...
\textbf{What the interviewer is testing:} Depth of understanding of emergent LLM capabilities and awareness of current research.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Knows ICL exists and that it's about pattern matching, not gradient updates
    \item \textbf{L6:} Can articulate 2--3 theories, discusses practical implications (format sensitivity, ordering)
    \item \textbf{L7:} Knows the ``attention as gradient descent'' formalism, discusses scale dependence, and connects to the mesa-optimization framework
\end{itemize}

\textbf{Common follow-ups:} ``Why does example order matter?'' ``Can ICL solve tasks not seen during pre-training?'' ``How do you decide between ICL and fine-tuning for a production task?''
\end{interviewq}
...
\textbf{Strong answer:} Tokenization is the first step in the NLP pipeline, and it profoundly affects sequence length, vocabulary size, rare word handling, and multilingual capability.
...
\textbf{What the interviewer is testing:} Understanding of tokenization fundamentals and their downstream effects on model design.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Describes each method correctly, knows the vocab-vs-length tradeoff
    \item \textbf{L6:} Discusses byte-level BPE, multilingual efficiency, and the impact on arithmetic/reasoning tasks
    \item \textbf{L7:} Connects tokenization to ``fertility'' (tokens per word varies by language, creating unfairness), discusses tokenizer training data biases, mentions byte-level models (MegaByte) as a potential solution
\end{itemize}

\textbf{Common follow-ups:} ``Why do LLMs struggle with character-level tasks like counting letters?'' ``How does vocabulary size affect model parameters?'' ``What happens if you fine-tune with a different tokenizer than pre-training?''
\end{interviewq}

\begin{interviewq}
\textbf{Q: Why do decoder-only models dominate modern NLP despite encoder-decoder being theoretically more flexible?}

\textbf{Difficulty:} L6 \quad \textbf{Type:} Conceptual \quad \textbf{Frequency:} \freqhigh{}

\textbf{Strong answer:} This is a crucial architectural question. Encoder-decoder models have a structural advantage for conditioned generation---the encoder can bidirectionally understand the input while the decoder generates the output. Yet decoder-only models (GPT, LLaMA, Claude) dominate. The reasons are primarily about scaling efficiency and interface design, not modeling capability.
...
\textbf{What the interviewer is testing:} Understanding of the systems-level and scaling-level reasons behind architectural trends, beyond just modeling capabilities.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Names the interface universality advantage
    \item \textbf{L6:} Discusses parameter efficiency, training data requirements, and infrastructure simplicity
    \item \textbf{L7:} Connects to the ``bitter lesson'' (simpler + scale > clever architecture), discusses prefix LM (decoder with bidirectional prefix) as a middle ground, mentions that Google's Gemini-family uses decoder-only despite having developed T5
\end{itemize}

\textbf{Common follow-ups:} ``What is a prefix language model and does it get the best of both?'' ``Why is Whisper encoder-decoder while LLMs are decoder-only?'' ``Could decoder-only models ever be as good at translation as encoder-decoder?''
\end{interviewq}
...
\textbf{Strong answer:} This is a complex system requiring multiple architectural decisions. I'll break it down by component.
...
\textbf{What the interviewer is testing:} System design thinking at the intersection of NLP, multilingual, and document AI.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Chooses a multilingual model and mentions layout information
    \item \textbf{L6:} Designs the full pipeline with OCR, layout-aware model, and task-specific heads; addresses data imbalance
    \item \textbf{L7:} Discusses cross-lingual transfer, temperature sampling for language balance, serving architecture, and evaluation strategy across languages (not just English accuracy)
\end{itemize}

\textbf{Common follow-ups:} ``How do you evaluate on languages you don't speak?'' ``What if a new language is added with zero training data?'' ``How does the system handle documents that mix tables and prose?''
\end{interviewq}
...
\textbf{Strong answer:} DPO (Direct Preference Optimization) is an alignment technique that trains directly on preference pairs without needing to train a reward model or run RL. Starting from the RLHF objective (maximize reward while staying close to reference policy), the key insight is that the optimal policy has a closed-form relationship to the reward function.
...
\textbf{What the interviewer is testing:} Mathematical understanding of alignment techniques and ability to derive results from first principles.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Knows DPO simplifies RLHF by eliminating the reward model, can state the loss
    \item \textbf{L6:} Derives the loss from the RLHF objective, explains the implicit reward, discusses $\beta$'s role
    \item \textbf{L7:} Discusses DPO variants (IPO, KTO, ORPO), the offline vs.\ online data limitation, and when RLHF is still preferable (e.g., when you need online data collection or reward model reuse across policies)
\end{itemize}

\textbf{Common follow-ups:} ``What is $\beta$ doing intuitively?'' ``When would you still prefer RLHF over DPO?'' ``What is KTO and how does it differ from DPO?''
\end{interviewq}
...
\textbf{Strong answer:} I'd approach this as a production classification system with several practical constraints.
...
\textbf{What the interviewer is testing:} Practical system design for NLP, balancing model quality, cost, and maintainability.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Chooses the right model family (encoder), justifies over LLM
    \item \textbf{L6:} Addresses taxonomy evolution, active learning, and evaluation
    \item \textbf{L7:} Designs the full production loop (model $\to$ monitoring $\to$ data collection $\to$ retraining), discusses cost analysis (encoder vs.\ LLM API), addresses the cold-start problem for new categories
\end{itemize}

\textbf{Common follow-ups:} ``How do you handle a new category with zero labeled examples?'' ``What's your retraining cadence?'' ``How do you detect when the model is underperforming?''
\end{interviewq}
...
\textbf{Strong answer:} Positional encoding is how transformers understand token order (since self-attention is permutation-invariant without it). The three approaches differ in expressiveness, generalization, and computational cost.
...
\textbf{What the interviewer is testing:} Understanding of positional encoding design choices and their practical implications for long-context models.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Describes each method and knows RoPE enables length extrapolation
    \item \textbf{L6:} Explains the rotation mechanism, relative position property, and why it's parameter-free
    \item \textbf{L7:} Discusses NTK-aware scaling, YaRN for extending context beyond training length, and the relationship between RoPE frequency base and effective context window
\end{itemize}

\textbf{Common follow-ups:} ``What is NTK-aware scaling for RoPE?'' ``How does ALiBi differ from RoPE?'' ``What happens when you test a RoPE model on 4$\times$ the training context length?''
\end{interviewq}