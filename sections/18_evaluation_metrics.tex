...
\textbf{What the interviewer is testing:} Whether you immediately question the metric itself before debugging the model. Jumping to model tuning is a strong negative signal.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Identifies class imbalance and metric mismatch, proposes alternatives
    \item \textbf{L6:} Covers all five causes, discusses calibration and per-segment analysis
    \item \textbf{L7:} Proposes a metric alignment process with stakeholders, discusses composite metrics and guardrails, mentions fairness implications of per-segment failures
\end{itemize}

\textbf{Common follow-ups:} ``How would you choose between precision and recall?'' ``What metric would you propose instead?'' ``How do you handle conflicting metric requirements?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} End-to-end evaluation thinking. Do you understand the full pipeline from offline to online? Do you know why offline gains do not always translate?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correct offline and online metrics, mentions the offline-online gap
    \item \textbf{L6:} Sliced evaluation, interleaving experiments, position bias awareness, guardrail metrics
    \item \textbf{L7:} Discusses counterfactual evaluation, unbiased learning-to-rank, long-term holdout experiments
\end{itemize}
...
\textbf{Strong answer:} First, determine whether this is actually a problem. If precision matters most (content moderation where false positives cause user outrage), 0.95/0.30 might be the right operating point. If recall matters most (fraud detection, medical screening), we are missing 70\% of positive cases---this is critical. To improve recall without destroying precision: \textbf{1. Lower the threshold}: Move from 0.5 to 0.3. Plot the full PR curve to find the optimal operating point for the business cost function. \textbf{2. Improve the model}: High precision/low recall often means the model learned only ``easy'' positives. Try hard negative mining, focal loss, oversampling rare positives, or adding discriminative features. \textbf{3. Cascade}: Use the high-precision model as stage one. Add a second, higher-recall model for cases the first misses. Route disagreements to human review. \textbf{4. Audit labels}: Low recall sometimes means many positives are mislabeled as negative. Sample 100 false negatives and verify ground truth.
...
\textbf{What the interviewer is testing:} Whether you reason structurally (business context first, then threshold, then model improvements) rather than mechanically.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Business context first, then threshold tuning and model improvements
    \item \textbf{L6:} Cascade approach, label audit, cost-sensitive optimization
    \item \textbf{L7:} Discusses the optimal decision boundary using expected cost analysis, proposes a human-in-the-loop system for the uncertain region
\end{itemize}

\textbf{Common follow-ups:} ``How do you choose the optimal threshold?'' ``What if stakeholders demand both $>$ 0.90?'' ``How does this change for real-time systems?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Production experimentation maturity. Sample size planning, multiple comparisons, offline vs.\ online testing.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Bootstrap CI for offline, power analysis for online, mentions multiple comparisons
    \item \textbf{L6:} McNemar's test, per-user aggregation, sequential testing for early stopping
    \item \textbf{L7:} Discusses always-valid p-values, variance reduction techniques (CUPED), and when A/B tests are fundamentally insufficient (marketplace effects requiring switchback)
\end{itemize}

\textbf{Common follow-ups:} ``Statistical vs.\ practical significance?'' ``How do you handle novelty effects?'' ``What if the test is inconclusive after 4 weeks?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Do you understand the distinction between discrimination and calibration? Do you know when calibration matters in production?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Explains discrimination vs.\ calibration, knows temperature scaling
    \item \textbf{L6:} Gives concrete examples where miscalibration causes business harm (ad bidding, medical), knows multiple calibration methods
    \item \textbf{L7:} Discusses calibration under distribution shift (calibration degrades faster than AUC), proposes online recalibration, mentions Platt scaling with fairness constraints
\end{itemize}

\textbf{Common follow-ups:} ``How do you calibrate a multi-class model?'' ``Does temperature scaling change the AUC?'' ``How do you validate calibration with limited data?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Do you know which metric matches which ranking scenario? Can you map task requirements to the right evaluation?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correct descriptions and when to use each, mentions the binary vs.\ graded distinction
    \item \textbf{L6:} Discusses retrieval vs.\ ranking stage metrics, knows NDCG edge cases (undefined when no relevant docs)
    \item \textbf{L7:} Connects to online metrics, discusses position bias in label collection, proposes unbiased offline evaluation methods
\end{itemize}

\textbf{Common follow-ups:} ``How do you handle queries with no relevant documents in NDCG?'' ``What is the relationship between MAP and area under the PR curve?'' ``Which metric is most sensitive to improvements at position 1?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Can you do back-of-envelope statistical calculations? Do you understand the relationship between effect size, variance, and sample size?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correct formula, clarifies relative vs.\ absolute, produces a reasonable number
    \item \textbf{L6:} Translates to duration given traffic, mentions variance reduction (CUPED), knows the minimum duration rule
    \item \textbf{L7:} Discusses sequential testing to enable early stopping, proposes metric sensitivity analysis, considers the business cost of running an underpowered test
\end{itemize}

\textbf{Common follow-ups:} ``How would you reduce the required sample size?'' ``What if you have only 10K daily users?'' ``What is CUPED and how does it help?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Can you design evaluation for an open-ended system where traditional metrics are insufficient? Do you understand the cost-reliability tradeoff?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Mentions automated metrics, human evaluation, and A/B tests
    \item \textbf{L6:} Three-tier framework, LLM-as-judge with bias mitigation, structured rubrics
    \item \textbf{L7:} Designs the full evaluation pipeline with gating between tiers, proposes contamination detection for benchmarks, discusses the philosophical challenge of evaluating alignment
\end{itemize}

\textbf{Common follow-ups:} ``How do you evaluate factuality?'' ``What are the biases of LLM-as-judge?'' ``How do you measure user satisfaction for a chatbot?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Deep understanding of why offline and online metrics diverge. This is a Staff-level signal---junior candidates often assume offline metrics are predictive.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Mentions 3+ causes including position bias and metric mismatch
    \item \textbf{L6:} All 5 causes with concrete examples, proposes counterfactual evaluation and interleaving
    \item \textbf{L7:} Designs offline evaluation methods that better predict online performance (unbiased estimators, propensity scoring), discusses when to abandon offline evaluation entirely
\end{itemize}

\textbf{Common follow-ups:} ``How would you make offline metrics more predictive of online?'' ``What is inverse propensity scoring?'' ``When should you skip offline eval and go straight to A/B?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Metric literacy. Can you articulate \textit{why} accuracy fails and when it does not? This is a foundational screening question.

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Explains the imbalance problem, knows when to use precision/recall instead, mentions valid use cases
    \item \textbf{L6:} Discusses error cost asymmetry, threshold dependence, multi-class averaging strategies
    \item \textbf{L7:} Connects to the broader metric selection framework, discusses how accuracy can mislead organizational decision-making (``our model is 95\% accurate'')
\end{itemize}

\textbf{Common follow-ups:} ``What metric would you use for a fraud detection system?'' ``How does accuracy relate to cross-entropy loss?'' ``When would you use macro F1 vs.\ weighted F1?''
\end{interviewq}
