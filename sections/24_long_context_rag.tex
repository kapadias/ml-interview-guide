...
\textbf{What the interviewer is testing:} Can you adapt RAG to a high-stakes domain with domain-specific constraints? Do you handle document structure, citation verification, and compliance?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Designs a basic RAG pipeline with legal-specific chunking.
    \item \textbf{L6:} Adds citation graph retrieval, hybrid search (BM25 critical for legal citations), faithfulness verification, and access control.
    \item \textbf{L7:} Designs an agentic retrieval loop for complex reasoning queries, builds a citation graph index, proposes domain-specific embedding fine-tuning with legal IR benchmarks, and addresses compliance requirements (audit logging, privilege, disclaimers).
\end{itemize}

\textbf{Common follow-ups:} ``How do you handle documents that update (amended statutes)?'' ``How do you evaluate citation accuracy?'' ``What if the user's query involves reasoning across 50 related cases?''
\end{interviewq}
...
\begin{itemize}
    ...
    \item Latency: 128K prefill at $\sim$1M tokens/sec (H100 optimized) $\approx$ 128ms prefill + $\sim$500 tokens at 50 tok/sec $\approx$ 10s decode $= \sim$10.1s total.
\end{itemize}
...
\begin{itemize}
    ...
    \item Latency: retrieval ($\sim$55ms) + LLM prefill ($\sim$3ms) + decode ($\sim$10s) $\approx$ 10.1s.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Latency:} Comparable for decode-dominated responses. For short answers ($<$100 tokens), long context has higher latency due to prefill.
...
\textbf{What the interviewer is testing:} Can you do back-of-envelope cost estimation and convert it to business terms? Do you know the key cost drivers (input tokens dominate for long context, infrastructure for RAG)?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correctly estimates that long context is significantly more expensive per query and gives approximate numbers.
    \item \textbf{L6:} Provides detailed per-query cost breakdowns for both approaches, converts to annual cost at scale, and discusses caching as a cost modifier.
    \item \textbf{L7:} Factors in quality differences (long context wins for cross-document reasoning, RAG wins for localized answers), discusses the Pareto frontier of cost vs.\ quality, and proposes a hybrid architecture that routes queries to the cheaper option based on query type.
\end{itemize}

\textbf{Common follow-ups:} ``How does caching change the cost for multi-turn conversations?'' ``What about latency comparison?'' ``When is the cost premium of long context justified?''
\end{interviewq}
...
\begin{enumerate}
    \item Check retrieval recall (cause in $\sim$70\% of failures).
...
\textbf{What the interviewer is testing:} Systematic evaluation methodology. Do you decompose into components? Do you know that retrieval is usually the bottleneck? Do you have a debugging protocol?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Evaluates retrieval and generation separately, knows Recall@$k$ and faithfulness.
    \item \textbf{L6:} Uses the full RAGAS framework, follows a structured debugging protocol (retrieval first), and designs production monitoring.
    \item \textbf{L7:} Discusses how to build the evaluation dataset (synthetic question generation from chunks + human validation), handles the ``no ground truth'' case (LLM-as-judge, implicit feedback signals), and proposes continuous improvement loops (failure cases feed back into embedding model fine-tuning and test set expansion).
\end{itemize}

\textbf{Common follow-ups:} ``How do you build the labeled dataset?'' ``What if faithfulness is high but users are unhappy?'' ``How do you evaluate when there is no ground truth?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Can you diagnose generation-stage failures when retrieval is working? Do you know the ``lost in the middle'' problem? Do you propose targeted fixes for each root cause?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Identifies parametric knowledge override and proposes prompt engineering fixes.
    \item \textbf{L6:} Diagnoses multiple root causes (parametric override, lost in the middle, semantic mismatch), proposes specific fixes for each, and mentions the ``read and extract'' intermediate step.
    \item \textbf{L7:} Discusses fine-tuning for grounded generation, reasons about the fundamental tension between parametric knowledge and context-faithfulness, and proposes a faithfulness monitoring system that detects context-ignorance in production.
\end{itemize}

\textbf{Common follow-ups:} ``How do you tell if the model is using parametric knowledge vs.\ hallucinating?'' ``What is the optimal number of chunks to pass?'' ``How do you fine-tune for better context grounding?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Do you recognize chunking as a critical design decision (not a trivial preprocessing step)? Can you match chunking strategies to document types? Do you know the parent-child pattern?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Describes fixed-size and document-aware approaches, recommends preserving sentence boundaries and using overlap.
    \item \textbf{L6:} Provides a decision framework mapping strategies to document types, explains the parent-child pattern, and mentions semantic chunking.
    \item \textbf{L7:} Discusses how chunk size interacts with embedding model training (most embedding models are trained on short passages---very long chunks degrade embedding quality), proposes adaptive chunk size based on information density, and designs an evaluation framework for comparing chunking strategies (measure retrieval recall and end-to-end accuracy across strategies).
\end{itemize}

\textbf{Common follow-ups:} ``What chunk size should you use?'' ``How do you handle tables that span multiple pages?'' ``How do you evaluate which chunking strategy is best?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Do you know when each technique helps and when it hurts? Can you match the technique to the failure mode it addresses?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correctly describes all three techniques and gives basic use cases.
    \item \textbf{L6:} Provides a failure-mode-based decision framework, discusses latency costs, and mentions combining techniques.
    \item \textbf{L7:} Discusses how to detect which technique is needed automatically (classify query type $\rightarrow$ route to appropriate processing), proposes caching HyDE results for repeated query patterns, and reasons about when the added LLM cost exceeds the retrieval quality benefit.
\end{itemize}

\textbf{Common follow-ups:} ``How do you decide whether to use query processing at all?'' ``What is the latency impact of HyDE?'' ``How do you evaluate the quality of query rewriting?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Nuanced engineering judgment. Can you reason about cost, latency, and quality simultaneously? Do you know the lost-in-the-middle problem? Do you recognize the hybrid approach?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correctly identifies corpus size and cost as the primary decision factors.
    \item \textbf{L6:} Provides concrete cost estimates, discusses lost-in-the-middle, and recommends the hybrid approach.
    \item \textbf{L7:} Factors in caching (prefix caching changes the cost equation for multi-turn), reasons about the latency difference ($O(n^2)$ attention for long context vs.\ fast prefill for RAG), and proposes an intelligent routing system that sends queries to long context or RAG based on query characteristics.
\end{itemize}

\textbf{Common follow-ups:} ``What is the lost-in-the-middle problem?'' ``How does caching change the cost comparison?'' ``What about latency at 128K tokens?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Can you diagnose a nuanced RAG failure that is not just ``bad retrieval''? Do you know the difference between factual retrieval and reasoning retrieval? Can you propose architectural solutions (agentic loops, query decomposition)?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Identifies that reasoning queries need multiple chunks and proposes retrieving more context.
    \item \textbf{L6:} Proposes query decomposition, agentic retrieval loops, and model routing (stronger model for reasoning queries). Separates retrieval from generation failures.
    \item \textbf{L7:} Designs a query classification system that routes factual vs.\ reasoning queries to different pipelines (factual: fast single-pass RAG; reasoning: agentic multi-hop with a stronger model), quantifies the cost-quality tradeoff, and discusses the fundamental limitation of single-vector retrieval for complex queries.
\end{itemize}

\textbf{Common follow-ups:} ``How do you classify queries as factual vs.\ reasoning?'' ``What is the latency cost of agentic RAG?'' ``Can you fine-tune the embedding model to handle multi-hop queries better?''
\end{interviewq}
...
\textbf{What the interviewer is testing:} Can you design a complete system with enterprise concerns (access control, multiple formats, domain adaptation, evaluation)? Not just ``embed and retrieve.''

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Designs a working RAG pipeline with chunking, embedding, and retrieval.
    \item \textbf{L6:} Adds access control, hybrid retrieval, domain fine-tuning, parent-child indexing, and structured evaluation.
    \item \textbf{L7:} Discusses incremental re-indexing for document updates, agentic retrieval for multi-hop enterprise questions, cost estimation for the vector database at 10M-document scale, and a feedback loop from production failures to embedding model improvement.
\end{itemize}

\textbf{Common follow-ups:} ``How do you handle frequently updated documents?'' ``How do you handle multimodal content (diagrams)?'' ``What if users ask questions spanning multiple documents?''
\end{interviewq}
...
\textbf{Bi-encoder:} Encodes query and document independently into single vectors. Similarity = cosine distance. \textit{Speed:} Fastest---document embeddings are pre-computed; only the query embedding is computed at query time. ANN search is $O(1)$ amortized. \textit{Quality:} Lowest---the single-vector bottleneck loses fine-grained token-level matching. \textit{Use for:} First-stage retrieval over the full corpus. Cannot be used for reranking (it already is the retrieval model).
...
\textbf{ColBERT (late interaction):} Encodes query and document independently (like bi-encoder) but retains per-token embeddings. Similarity = MaxSim (sum of maximum per-token similarities). \textit{Speed:} Medium---document token embeddings are pre-computed. Query-time computation is lightweight MaxSim. 10--100$\times$ faster than cross-encoder. \textit{Quality:} Near cross-encoder quality---token-level matching without joint encoding. \textit{Storage cost:} High---one embedding per token (vs.\ one per document for bi-encoder). A 512-token document at 128 dims stores 64KB vs.\ 512 bytes for a bi-encoder (125$\times$ more). \textit{Use for:} Reranking when cross-encoder latency is too high, or as a fast first-stage retrieval with higher quality than bi-encoder.
...
\textbf{What the interviewer is testing:} Do you understand the speed-quality-storage tradeoff across retrieval and reranking models? Can you design a multi-stage pipeline that balances these factors?

\textbf{What separates good from great:}
\begin{itemize}
    \item \textbf{L5:} Correctly explains that bi-encoders are fast/cheap and cross-encoders are slow/accurate.
    \item \textbf{L6:} Describes ColBERT's late interaction mechanism, quantifies the storage tradeoff, and designs a multi-stage pipeline with explicit latency budgets.
    \item \textbf{L7:} Discusses when to add BM25 as a complementary sparse retrieval stage (hybrid retrieval), reasons about the crossover point where ColBERT's storage cost exceeds the value of faster reranking, and proposes ColBERT with quantized token embeddings to reduce storage.
\end{itemize}

\textbf{Common follow-ups:} ``How do you decide the number of candidates for reranking?'' ``Can you combine BM25 with dense retrieval?'' ``How do you evaluate reranking quality?''
\end{interviewq}
