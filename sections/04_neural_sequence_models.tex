...
\textbf{Concat (same functional form as Bahdanau, but applied to the
...
Decoder state used & Previous state $\vh_{t-1}^d$ ($s_{t-1}$) & Current top-layer state $\vh_t^d$ \\
...
\item \textbf{Convolutional filters:} Apply 1D convolutions with multiple
  filter widths, e.g.\ $h \in \{3, 4, 5\}$ as in Kim (2014).  A filter of
  width $h$ slides
