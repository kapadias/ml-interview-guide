...
\textbf{When to use each version:}
\begin{itemize}
    \item \textbf{MobileNetV1}: When extreme simplicity is needed (easy to implement, port, and debug) or as a teaching baseline. Outclassed by V2/V3 on the accuracy-latency Pareto frontier.
    \item \textbf{MobileNetV2}: The best general-purpose mobile backbone. Excellent trade-off between accuracy, latency, and implementation complexity. Widely supported across all mobile inference runtimes. The default choice for most mobile deployment scenarios.
    \item \textbf{MobileNetV3}: When maximum accuracy within a mobile latency budget is required and the deployment target supports all required operators (SE, h-swish). Particularly strong for detection and segmentation backbones (MobileNetV3 + SSDLite, MobileNetV3 + DeepLabV3).
\end{itemize}
...
\begin{itemize}
    ...
    \item \badgeLsix{L6} Provides the decision framework above. Understands the difference between weight-only quantization (W4A16) and weight+activation quantization (W8A8). Knows that attention layers are more sensitive to quantization than FFN layers. Discusses FP8's per-tensor scaling mechanism.
